...
if (winner.plasticity === undefined) throw new Error("B5's winner must configure plasticity for this measurement to mean anything");

const amplitude: LevelMapConfig = { channel: NORADRENALINE, reference: 1.0, gain: 0.8, min: -0.5, max: 4.0 };
const timing: LevelMapConfig = { channel: NORADRENALINE, reference: 1.0, gain: 0.6, min: 0.25, max: 4.0 };
...
const base: PlasticityConfig = { ...winner.plasticity, modulatorTauTicks: noDecay };
const modulated: PlasticityConfig = { ...base, stdpModulation: { aPlus: amplitude, aMinus: amplitude, tauPlus: timing, tauMinus: timing, windowTicks: timing } };

const held = { extraTonicModulators: [{ channel: NORADRENALINE, level: 1.0 }] } as const;
const arms: { readonly name: string; readonly config: CharPredictionConfig }[] = [
  { name: "hook unset", config: { ...winner, ...held, plasticity: base } },
  { name: "hook set, all five slots, at reference", config: { ...winner, ...held, plasticity: modulated } },
];
...
function runOnce(config: CharPredictionConfig): Run {
  ...
  return { seconds: (performance.now() - started) / 1000, accuracy: result.networkAccuracy, observation: observation! };
}

const median = (xs: readonly number[]) => [...xs].sort((a, b) => a - b)[Math.floor(xs.length / 2)]!;

console.log(`corpus ${CHARS} characters, seed ${SEED}, ${REPS} alternating repetitions per arm (one warm-up pair discarded)\n`);
...
for (const [i, arm] of arms.entries()) {
  console.log(`${arm.name.padEnd(42)} median ${median(times[i]!).toFixed(3)} s   all: ${times[i]!.map((t) => t.toFixed(3)).join(", ")}`);
}
...
const a = last[0]!;
const b = last[1]!;
const same = a.observation.permanenceHash === b.observation.permanenceHash && a.observation.weightHash === b.observation.weightHash && a.observation.topologyHash === b.observation.topologyHash && a.accuracy === b.accuracy;
console.log(`\nBIT-IDENTITY through the FFI on the real network: ${same ? "PASS" : "FAIL"}`);
console.log(`  permanence hash  ${a.observation.permanenceHash} vs ${b.observation.permanenceHash}`);
console.log(`  weight hash      ${a.observation.weightHash} vs ${b.observation.weightHash}`);
console.log(`  topology hash    ${a.observation.topologyHash} vs ${b.observation.topologyHash}`);
