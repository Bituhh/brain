...
const workers = Math.max(1, Math.min(Number(process.env.C7_WORKERS ?? 12), cpus().length));
...
const ratioMap = (reference: number, gain: number, floor: number): LevelMapConfig => ({ channel: ACETYLCHOLINE, reference, gain: -gain, min: floor, max: 1.0 });
...
const inductionOnly = (map: LevelMapConfig): CharPredictionConfig => ({
  ...gateMoved,
  plasticity: { ...gateMoved.plasticity!, stdpModulation: { aPlus: map }, observeStdpModulation: true },
...
const shared = (map?: LevelMapConfig): CharPredictionConfig => ({
  ...winnerUnheld,
  ...(map !== undefined && { plasticity: { ...plasticity, stdpModulation: { aPlus: map }, observeStdpModulation: true } }),
  predictionErrorCoupling: achCoupling,
});
...
const phase1: Job[] = [
  ...
  ...SEEDS.map((seed) => ({ key: keyOf(measureConfig, seed), label: "M: induction-only, map gain 0 (measures reference)", seed, config: measureConfig })),
  ...SEEDS.map((seed) => ({ key: keyOf(variesConfig, seed), label: "V: ACh driven, shared wiring, no map (C2's row)", seed, config: variesConfig })),
];
log(`=== investigate-c7-ach-ratio: ${workers} workers, ${CORPUS_LENGTH} characters, seeds ${SEEDS.join(", ")} ===`);
...
const ARMS = [
  { name: "INV3", label: "INV3: g 3, floor -1 (inverts)", config: inductionOnly(ratioMap(reference, 3, -1.0)) },
  { name: "SUP3", label: "SUP3: g 3, floor 0 (twin: suppresses only)", config: inductionOnly(ratioMap(reference, 3, 0.0)) },
  { name: "INV1.5", label: "INV1.5: g 1.5, floor -1 (low dose)", config: inductionOnly(ratioMap(reference, 1.5, -1.0)) },
  { name: "SHARED3", label: "SHARED3: INV3's map on the shipped wiring", config: shared(ratioMap(reference, 3, -1.0)) },
] as const;
const heldConfig = held(ratioMap(reference, 3, -1.0), reference);
const phase2: Job[] = [
  ...HASH_SEEDS.map((seed) => ({ key: keyOf(heldConfig, seed), label: "X: INV3's map, ACh held exactly at reference", seed, config: heldConfig })),
  ...ARMS.flatMap((arm) => SEEDS.map((seed) => ({ key: keyOf(arm.config, seed), label: arm.label, seed, config: arm.config }))),
];
await runAll(phase2);
...
out.push(`# PLAN.md C7 -- acetylcholine sets the LTP/LTD ratio: the pre-registered VAL-4 measurement`);
out.push(``);
...
out.push(`seeds 1 / 2 / 3 ${[1n, 2n, 3n].map(refAcc).map(pct).join(" / ")} (B5: 19.85 / 20.50 / 21.10%).`);
out.push(``);
let pass = true;
...
for (const seed of SEEDS) {
  ...
  if (m) check(sameStructure(m, referenceRecord(seed)!), `M seed ${seed} (ACh driven, read only by a gain-0 map) vs checkpointed reference`);
}
for (const seed of HASH_SEEDS) {
  ...
  for (const [name, config] of [["G", gateMoved], ["M", measureConfig], ["X (INV3's map, ACh held exactly at the reference)", heldConfig]] as const) {
...
out.push(``);
const accOf = (config: CharPredictionConfig | "R", seed: bigint, label: string) => (config === "R" ? refAcc(seed) : need(config, seed, label)?.accuracy);
const COMPARISONS: readonly { readonly title: string; readonly a: CharPredictionConfig; readonly aName: string; readonly b: CharPredictionConfig | "R"; readonly bName: string }[] = [
  { title: "1. INV3 vs R -- the modulator-driven ratio against the tuned constant", a: ARMS[0].config, aName: "INV3", b: "R", bName: "R" },
  { title: "2. INV3 vs SUP3 -- the sign inversion itself", a: ARMS[0].config, aName: "INV3", b: ARMS[1].config, bName: "SUP3" },
  { title: "3. INV1.5 vs R -- the low dose", a: ARMS[2].config, aName: "INV1.5", b: "R", bName: "R" },
  { title: "4. V vs R -- the channel now varies (shared wiring, no map)", a: variesConfig, aName: "V", b: "R", bName: "R" },
  { title: "5. SHARED3 vs V -- the channel now does something, on the shipped wiring", a: ARMS[3].config, aName: "SHARED3", b: variesConfig, bName: "V" },
];
...
for (const arm of ARMS) {
  for (const seed of SEEDS) {
    ...
    if (!s) continue;
    out.push(`| ${arm.name} | ${seed} | ${s.events} | ${((s.curveChanged / s.events) * 100).toFixed(2)}% | ${((s.amplitudeInverted / s.events) * 100).toFixed(3)}% | ${s.minScale.toFixed(4)} |`);
  }
}
out.push(``);
out.push(`"Pairings" is every STDP kernel evaluation over the run; "sign inverted" the share at which a causal pairing laid down depression.`);
out.push(``);
out.push(`## Verdict`);
out.push(``);
out.push(...verdicts);
const inv3 = SEEDS.map((s) => done.get(keyOf(ARMS[0].config, s))?.accuracy).filter((a): a is number => a !== undefined);
if (inv3.length > 0) out.push(`- Lowest INV3 seed: ${pct(Math.min(...inv3))} against the ${pct(ALWAYS_SPACE)} bar.`);
if (missing.length > 0) {
  out.push(``);
