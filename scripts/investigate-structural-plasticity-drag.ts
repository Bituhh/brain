...
const RESULTS_PATH = fileURLToPath(
  new URL(
    './investigate-structural-plasticity-drag.results.md',
    import.meta.url,
  ),
);
...
const CONDITIONS: readonly Condition[] = [
  ...
  },
  {
    ...
    description:
      "minActivityStreak set unreachably high (max possible streak over a 15,000-char/~30,000-tick run at sweepIntervalTicks=200 is ~150) so sprout's own eligibility check never passes -- isolates whether pruning alone, with no new synapses ever created, is safe on its own.",
    config: {
      ...DEFAULT_CONFIG,
      structuralPlasticity: {
        ...baseStructuralPlasticityParams(),
        minActivityStreak: 10_000,
      },
    },
  },
  {
    ...
    description:
      "Tests whether more aggressive floor-based pruning alone -- with sprout's placement logic (symmetric/atemporal, always segment 0, weight-blind on dendrites) otherwise unchanged -- mitigates the drag, as a config-only stopgap distinct from PLAN.md B4's actual mechanism fixes.",
    config: {
      ...DEFAULT_CONFIG,
      structuralPlasticity: {
        ...baseStructuralPlasticityParams(),
...
function logResult(
  ...
  appendFileSync(
    RESULTS_PATH,
    `| ${name} | ${(meanAccuracy * 100).toFixed(2)}% | ${range} | ${(wallClockMs / 1000).toFixed(1)}s |\n`,
  );
}
