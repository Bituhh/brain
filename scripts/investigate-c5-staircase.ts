...
const CORPUS_LENGTH = Number(
  process.env.C5_CHARS ?? (LONG_G !== undefined ? 15000 : 6000),
);
const SEEDS = [1n, 2n, 3n] as const;
const workers = Math.max(
  1,
  Math.min(Number(process.env.C5_WORKERS ?? 14), cpus().length),
);
...
const grid = (lo: number, hi: number, step: number): number[] => {
  ...
  return Array.from(
    { length: n + 1 },
    (_, i) => Math.round((lo + i * step) * 1e6) / 1e6,
  );
};
...
const s0: Row[] = [
  {
    sweep: 'S0',
    g: null,
    name: 'no reward signal (the reference)',
    config: winner,
  },
  {
    sweep: 'S0',
    g: null,
    name: 'raw reward (no baseline)',
    config: { ...winner, rewardSignal: 'correctness' },
  },
  ...[50, 200, 1000].map((tau): Row => ({
    sweep: 'S0',
    g: tau,
    name: `RPE tauEvents ${tau}`,
    config: {
      ...winner,
      rewardSignal: 'correctness',
      rewardPredictionError: rpe(tau),
    },
  })),
];
...
const s2: Row[] = S23_GRID.map((g) => ({
  sweep: 'S2',
  g,
  name: `a_minus x ${g}`,
  config: withHook(g, { aMinus: ratioMap }),
}));
const s3: Row[] = S23_GRID.map((g) => ({
  sweep: 'S3',
  g,
...
const controls: Row[] = [
  {
    sweep: 'control',
    g: 1.0,
    name: 'hook UNSET, noradrenaline held at 1.0 (must equal S2 and S3 at g = 1.0)',
...
const ROWS: readonly Row[] =
  LONG_G !== undefined
    ? [...s2.filter((r) => LONG_G.includes(r.g!)), ...controls]
    : [...s0, ...s1, ...s2, ...s3, ...m2, ...m3, ...controls];
...
log(
  `=== investigate-c5-staircase: ${workers} workers, ${CORPUS_LENGTH} characters, seeds ${SEEDS.join(', ')} ===`,
);
log(
...
const rec = (row: Row, seed: bigint): Record | undefined =>
  done.get(keyOf(row.config, seed));
...
function statsFor(rows: readonly Row[], seed: bigint): SweepStats | undefined {
  const pts = rows
    .map((r) => ({ g: r.g!, r: rec(r, seed) }))
    .filter((p): p is { g: number; r: Record } => p.r !== undefined)
    .sort((a, b) => a.g - b.g);
  if (pts.length < 3) return undefined;
  const adj = (f: (r: Record) => string | number) =>
    pts.slice(1).filter((p, i) => f(p.r) === f(pts[i]!.r)).length;
  let longest = 1;
  let run = 1;
  ...
  const diffs = pts
    .slice(1)
    .map((p, i) => Math.sign(p.r.connected - pts[i]!.r.connected))
    .filter((s) => s !== 0);
  const reversals = diffs.slice(1).filter((s, i) => s !== diffs[i]).length;
  const range = (f: (r: Record) => number): [number, number] => [
    Math.min(...pts.map((p) => f(p.r))),
    Math.max(...pts.map((p) => f(p.r))),
  ];
  ...
  return {
    ...
    rhoConnected: spearman(
      gs,
      pts.map((p) => p.r.connected),
    ),
    rhoSumPermanence: spearman(
      gs,
      pts.map((p) => p.r.sumPermanence),
    ),
    rhoSumWeight: spearman(
      gs,
      pts.map((p) => p.r.sumWeight),
    ),
    rhoAccuracy: spearman(
      gs,
      pts.map((p) => p.r.accuracy),
    ),
...
out.push(``);
out.push(
  `Generated ${new Date().toISOString()}. ${CORPUS_LENGTH} characters per trial, seeds ${SEEDS.join(', ')}, B5's winner as the base (docs/decisions.md decision 13).`,
);
out.push(``);
out.push(
...
out.push(
  `(permanence >= ${winner.plasticity ? 'connectionThreshold' : ''}), \`perm\` every occupied synapse's permanence bits, \`weight\` likewise. Identical hashes mean identical to the synapse.`,
);
...
for (const sweep of ['S1', 'S2', 'S3'] as const) {
  ...
  for (const seed of SEEDS) {
    ...
    if (r)
      out.push(
        `| ${sweep} (g=${mid.g}) | ${seed} | ${r.outcomes.correct} | ${r.outcomes.falsePositive} | ${r.outcomes.unpredicted} | **${r.outcomes.classifiedAsPredicted}** |`,
      );
  }
}
...
for (const seed of SEEDS) {
  for (const row of s0) {
    ...
    if (r)
      out.push(
        `| ${seed} | ${row.name} | ${pct(r.accuracy)} | ${r.connected} | ${r.sumPermanence.toFixed(3)} | \`${r.topologyHash}\` | \`${r.permanenceHash}\` | \`${r.weightHash}\` | ${r.structural?.sproutedTotal ?? '-'} | ${r.structural?.prunedTotal ?? '-'} |`,
      );
  }
}
...
for (const [sweep, title, rows] of [
  ...
  for (const row of [...rows].sort((a, b) => a.g! - b.g!)) {
    ...
    if (r)
      out.push(
        `| ${row.g} | ${r.occupied} | ${r.connected} | ${r.sumPermanence.toFixed(3)} | ${r.sumWeight.toFixed(3)} | ${r.distinctPermanences} | ${r.atOne} | ${r.atZero} | \`${r.topologyHash}\` | \`${r.permanenceHash}\` | \`${r.weightHash}\` | ${pct(r.accuracy)} |`,
      );
  }
}
...
out.push(
  `points chosen at random, i.e. a ratio near 1/3. Accuracy is the harness's 2,000-character sliding window at the end of the run.`,
);
out.push(``);
out.push(
...
for (const [sweep, rows] of [
  ...
  for (const seed of SEEDS) {
    ...
    const meanAbsStep = (f: (r: Record) => number) =>
      pts
        .slice(1)
        .reduce((acc, p, i) => acc + Math.abs(f(p.r) - f(pts[i]!.r)), 0) /
      (pts.length - 1);
    const span = (f: (r: Record) => number) =>
      Math.max(...pts.map((p) => f(p.r))) - Math.min(...pts.map((p) => f(p.r)));
    const acc = (r: Record) => r.accuracy * 100;
    const w = (r: Record) => r.sumWeight;
    const ratio = (f: (r: Record) => number) =>
      span(f) === 0 ? 'n/a (flat)' : (meanAbsStep(f) / span(f)).toFixed(3);
    out.push(
      `| ${sweep} | ${seed} | ${span(acc).toFixed(2)} points | ${meanAbsStep(acc).toFixed(2)} | ${ratio(acc)} | ${span(w).toFixed(1)} | ${meanAbsStep(w).toFixed(1)} | ${ratio(w)} |`,
    );
  }
}
out.push(``);
...
for (const [sweep, micro, grid] of [
  ...
  for (const seed of SEEDS) {
    ...
    const around = [
      ...grid.filter((r) => r.g === 0.975 || r.g === 1.025),
      ...micro,
    ].sort((a, b) => a.g! - b.g!);
...
for (const seed of SEEDS) {
  ...
  for (const [name, row] of [
    ['S2', s2.find((r) => r.g === 1.0)!],
    ['S3', s3.find((r) => r.g === 1.0)!],
  ] as const) {
    const r = rec(row, seed);
    if (!control || !r) continue;
    const same =
      control.permanenceHash === r.permanenceHash &&
      control.weightHash === r.weightHash &&
      control.topologyHash === r.topologyHash &&
      control.accuracy === r.accuracy;
    controlsPass &&= same;
    out.push(
      `- seed ${seed}, ${name} at g = 1.0 vs hook unset: **${same ? 'PASS' : 'FAIL'}**`,
    );
  }
}
out.push(``);
out.push(
  controlsPass
    ? `All controls PASS.`
    : `**A CONTROL FAILED -- do not read the sweeps above until this is explained.**`,
);
