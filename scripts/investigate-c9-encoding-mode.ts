...
const workers = Math.max(
  1,
  Math.min(Number(process.env.C9_WORKERS ?? 12), cpus().length),
);
...
if (winner.voteReferenceWeight === undefined)
  throw new Error(
    "B5's winner must use a weighted dendritic vote, or the transmission gate cannot reach any segment",
  );
...
const transmissionMap = (reference: number, gain: number): LevelMapConfig => ({
...
const plasticityMap = (reference: number, gain: number): LevelMapConfig => ({
  channel: ACETYLCHOLINE,
  reference,
  gain,
  min: 1.0,
...
const recurrentRule = (map: LevelMapConfig | undefined) => {
  const {
    modulatorTauTicks: _tau,
    recurrent: _rec,
    ...rule
  } = gateMoved.plasticity!;
  return {
    ...rule,
    ...(map !== undefined && { stdpModulation: { aPlus: map } }),
...
const arm = (
  transmission: LevelMapConfig | undefined,
  plasticityGate: LevelMapConfig | undefined,
  alsoFeedforward = false,
  ...
  ...(transmission !== undefined && {
    transmissionModulation: {
      ...
      ...(alsoFeedforward && { feedforward: transmission }),
    },
  }),
...
const held = (
  ...
  ...(transmission !== undefined && {
    transmissionModulation: { recurrent: transmission },
  }),
...
const phase1: Job[] = [
  ...
  ...SEEDS.map((seed) => ({
    key: keyOf(offConfig, seed),
    label: 'OFF: ACh driven, both maps at gain 0 (measures the reference)',
    seed,
    config: offConfig,
  })),
  ...HASH_SEEDS.map((seed) => ({
    key: keyOf(ffConfig, seed),
    label: 'FF: OFF plus a gain-0 map on the feedforward role',
    seed,
    config: ffConfig,
  })),
];
log(
  `=== investigate-c9-encoding-mode: ${workers} workers, ${CORPUS_LENGTH} characters, seeds ${SEEDS.join(', ')} ===`,
);
...
const ARMS = [
  {
    name: 'T1',
    label: 'T1: transmission only, gT 1.0',
    config: arm(T1, zeroP),
  },
  {
    name: 'T05',
    label: 'T05: transmission only, gT 0.5',
    config: arm(T05, zeroP),
  },
  { name: 'P1', label: 'P1: plasticity only, gP 1.0', config: arm(zeroT, P1) },
  {
    name: 'TP',
    label: 'TP: both halves, gT 1.0 + gP 1.0',
    config: arm(T1, P1),
  },
] as const;
...
const phase2: Job[] = [
  ...HASH_SEEDS.map((seed) => ({
    key: keyOf(heldMapped, seed),
    label: 'X: both maps live, ACh held exactly at the reference',
    seed,
    config: heldMapped,
  })),
  ...HASH_SEEDS.map((seed) => ({
    key: keyOf(heldPlain, seed),
    label: 'X-twin: gain-0 maps, ACh held at the same level',
    seed,
    config: heldPlain,
  })),
  ...ARMS.flatMap((a) =>
    SEEDS.map((seed) => ({
      key: keyOf(a.config, seed),
      label: a.label,
      seed,
      config: a.config,
    })),
...
out.push(``);
out.push(
  `Generated ${new Date().toISOString()}. ${CORPUS_LENGTH} characters per trial, B5's winner as the base, accuracy is the harness's`,
);
out.push(
  `2,000-character sliding window at the end of the run. Every choice below was written into the script header before any trial ran.`,
);
...
for (const seed of SEEDS) {
  ...
  if (o)
    check(
      sameStructure(o, referenceRecord(seed)!),
      `OFF seed ${seed} (ACh driven, read only by gain-0 maps) vs checkpointed reference`,
    );
}
for (const seed of HASH_SEEDS) {
  ...
  if (x && twin)
    check(
      sameBits(x, twin),
      `X seed ${seed} (both maps live, ACh held at the reference) vs its gain-0 twin, bit for bit`,
    );
}
out.push(``);
out.push(
  pass
    ? `All controls PASS.`
    : `**A CONTROL FAILED -- section 4 must not be read until this is explained.**`,
);
...
out.push(``);
out.push(
  `VAL-4 stimulates its column directly (\`stimulateSdr\`) and wires its whole recurrent web onto dendritic segments, so it has`,
);
out.push(
  `almost no feedforward *synapses* to spare. The FF row is OFF plus a gain-0 map on the **feedforward** role as well, so its`,
);
out.push(
  `\`events\` count is every delivery of either role; OFF's is the recurrent ones alone. The difference is the spared pathway's size.`,
);
out.push(``);
out.push(
...
for (const seed of HASH_SEEDS) {
  ...
  if (!o || !f) continue;
  out.push(
    `| ${seed} | ${o.events} | ${f.events} | ${f.events === 0 ? 'n/a' : `${(((f.events - o.events) / f.events) * 100).toFixed(3)}%`} |`,
  );
}
out.push(``);
out.push(
  `So "leaving feedforward delivery untouched" is close to vacuous here, exactly as PLAN.md C9's prompt anticipated. The`,
);
out.push(
  `spared-pathway contrast itself is asserted on a network that has both pathways, in \`crates/brain-core/tests/transmission_modulation.rs\`.`,
);
...
out.push(``);
const accOf = (config: CharPredictionConfig, seed: bigint, label: string) =>
  need(config, seed, label)?.accuracy;
const COMPARISONS: readonly {
  readonly title: string;
  readonly a: CharPredictionConfig;
  readonly aName: string;
  readonly b: CharPredictionConfig;
  readonly bName: string;
}[] = [
  {
    title: '1. TP vs OFF -- the pair the evidence describes',
    a: ARMS[3].config,
    aName: 'TP',
    b: offConfig,
    bName: 'OFF',
  ...
  {
    title: '2. T1 vs OFF -- the transmission half alone',
    a: ARMS[0].config,
    aName: 'T1',
    b: offConfig,
    bName: 'OFF',
  ...
  {
    title: '3. P1 vs OFF -- the plasticity half alone',
    a: ARMS[2].config,
    aName: 'P1',
    b: offConfig,
    bName: 'OFF',
  ...
  {
    title: '5. T05 vs OFF -- the half dose',
    a: ARMS[1].config,
    aName: 'T05',
    b: offConfig,
    bName: 'OFF',
...
for (const a of ARMS) {
  for (const seed of SEEDS) {
    ...
    if (!s) continue;
    out.push(
      `| ${a.name} | ${seed} | ${s.events} | ${((s.curveChanged / Math.max(1, s.events)) * 100).toFixed(2)}% | ${s.minScale.toFixed(4)} | ${s.maxScale.toFixed(4)} |`,
    );
  }
}
...
out.push(...verdicts);
const tp = SEEDS.map(
  (s) => done.get(keyOf(ARMS[3].config, s))?.accuracy,
).filter((a): a is number => a !== undefined);
if (tp.length > 0)
  out.push(
    `- Lowest TP seed: ${pct(Math.min(...tp))} against the ${pct(ALWAYS_SPACE)} bar.`,
  );
if (missing.length > 0) {
  out.push(``);
  out.push(
