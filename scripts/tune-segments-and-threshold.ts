...
const RESULTS_PATH = fileURLToPath(
  new URL('./tune-segments-and-threshold.results.md', import.meta.url),
);
writeFileSync(
  RESULTS_PATH,
...
function logTrial(
  ...
  appendFileSync(
    RESULTS_PATH,
    `| ${segmentsPerNeuron} | ${targetRate.toFixed(4)} | ${seedCount} | ${(meanAccuracy * 100).toFixed(2)}% | ${range} | ${trigram} | ${note} |\n`,
  );
}
...
function measure(
  ...
  const trials = runCharPredictionTrials(
    corpus,
    [...OFFICIAL_SEEDS],
    configFor(segmentsPerNeuron, targetRate),
  );
...
function searchTargetRateFor(segmentsPerNeuron: number): Evaluation {
  ...
  if (trialsThisAxis >= MAX_TRIALS_PER_AXIS) {
    console.log(
      `  Stopped segments=${segmentsPerNeuron} at MAX_TRIALS_PER_AXIS=${MAX_TRIALS_PER_AXIS} (safety valve) rather than true convergence.`,
    );
  }
  return best;
}
...
console.log(
  `Searching segmentsPerNeuron in {${SEGMENTS_VALUES.join(', ')}}, each with its own targetRate coordinate search from ${STARTING_TARGET_RATE}, bounds [${LOWER_BOUND}, ${UPPER_BOUND}].`,
);
console.log(
  `Every trial uses the full ${OFFICIAL_SEEDS.length}-seed official protocol.\n`,
);
...
for (const segmentsPerNeuron of SEGMENTS_VALUES) {
  ...
  logTrial(
    ...
    [],
    0,
    '**best for this segmentsPerNeuron (already logged above; restated for the summary table)**',
  );
  console.log(
    `  best for segmentsPerNeuron=${segmentsPerNeuron}: targetRate=${best.targetRate.toFixed(4)}, mean network accuracy=${(best.score * 100).toFixed(2)}%`,
  );
}
...
console.log(
  `Compare against DEFAULT_CONFIG's current segmentsPerNeuron=2, targetRate=${BASE_HOMEOSTASIS.targetRate} (docs/findings.md finding 7/9's 17.37%).`,
);
