...
import type { GrowthConfig, NewbornMaturationConfig, StructuralStats } from "@brain/core";
...
import { NETWORK_WIDTH, type CharPredictionConfig } from "../packages/io/src/milestone/charPrediction.ts";
...
function withGrowth(growth: GrowthConfig, restrictSources: boolean): CharPredictionConfig {
  return {
    ...winnerConfig,
    growth,
    newbornMaturation: newbornMaturationParams(),
    structuralPlasticity: { ...winnerConfig.structuralPlasticity!, ...(restrictSources && { maxSproutSourceIndex: WIDTH - 1 }) },
  };
}
...
const ROWS: readonly Row[] = [
  { name: "A: no growth, no structural plasticity, count mode", config: toConfig({ kind: "A-count" }) },
  { name: "C: structural plasticity at the B5 winner, no growth", config: winnerConfig },
  { name: "B: C + growth, burst pace", config: withGrowth(growthBurst(), false) },
  { name: "D: C + growth, burst pace, sprout-source-restricted", config: withGrowth(growthBurst(), true) },
  { name: "E: C + growth, gentle pace", config: withGrowth(growthGentle(), false) },
  { name: "F: C + growth, gentle pace, sprout-source-restricted", config: withGrowth(growthGentle(), true) },
];
...
const recordOf = (key: string) => (checkpoint.hasSucceeded(key) ? checkpoint.get(key) : searchCheckpoint.hasSucceeded(key) ? searchCheckpoint.get(key) : undefined);

log(`=== investigate-b5-growth starting: ${workers} workers, corpus ${CORPUS_LENGTH} characters, seeds ${SEEDS.join(", ")} ===`);
...
for (const row of ROWS) {
  for (const seed of SEEDS) {
    ...
    if (recordOf(key) === undefined) jobs.push({ key, label: row.name.slice(0, 1), seed, payload: row.config });
  }
}
  ...
  const statsCell =
    stats.length === 0
      ...
      : [mean(stats.map((s) => s.sproutedTotal)), mean(stats.map((s) => s.unsilencedTotal)), mean(stats.map((s) => s.eliminatedTotal)), mean(stats.map((s) => s.silentNow))]
          .map((x) => Math.round(x))
          .join(" / ");
  lines.push(`| ${row.name} | ${pct(mean(accuracies))} | ${accuracies.map(pct).join(", ")} | ${statsCell} | ${Math.round(mean(records.map((r) => r!.seconds)))} |`);
}
