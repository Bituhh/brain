...
const trajPath = join(
  process.env.C7_TRAJ_DIR ?? tmpdir(),
  `c7-ach-trajectory-${seed}.json`,
);
...
const configs: Record<string, CharPredictionConfig> = {
  ...
  record: {
    ...
    plasticity: {
      ...
      stdpModulation: { aPlus: { ...inv3, gain: 0 } },
    },
...
if (config === undefined)
  throw new Error(`mode must be record, open or closed, got ${mode}`);
const replay =
  mode === 'open'
    ? (JSON.parse(readFileSync(trajPath, 'utf8')) as number[])
    : [];
...
const result = runCharPredictionTrial(
  ...
  (sim: Simulation) => {
    if (mode === 'open') {
      ...
      sim.injectModulator(
        ACETYLCHOLINE,
        target - sim.modulatorLevels()[ACETYLCHOLINE]!,
      );
    }
...
console.log(
  JSON.stringify({
    ...
    achMedianByThird: [
      median(levels.slice(0, third)),
      median(levels.slice(third, 2 * third)),
      median(levels.slice(2 * third)),
