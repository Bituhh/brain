...
const at = (b: number): CharPredictionConfig => ({
  ...
  rewardPredictionError: { tauEvents: 200, drive: { channel: 0, baseline: b, gain: 0.0, maxLevel: 4.0 } },
});
...
const above = run(1.0);
console.log(`seed ${SEED}, ${CHARS} characters; connection threshold ${above.threshold}`);
console.log(`0.35 - 0.05 x 1.0 in f32 = ${Math.fround(Math.fround(0.35) - Math.fround(0.05))}  (threshold ${Math.fround(0.3)}; below it? ${Math.fround(Math.fround(0.35) - Math.fround(0.05)) < Math.fround(0.3)})`);
console.log(`0.35 - 0.05 x 0.998 in f32 = ${Math.fround(Math.fround(0.35) - Math.fround(0.05 * 0.998))}  (the level has decayed ~0.2% by the time it is read)`);
...
for (let i = 0; i < above.occupied.length; i++) {
  if (below.occupied[i] !== above.occupied[i]) console.log(`slot ${i}: occupancy differs`);
  ...
  if (wDiff) {
    differing++;
    targetsOfDifferingWeights.set(above.target[i]!, (targetsOfDifferingWeights.get(above.target[i]!) ?? 0) + 1);
    if (!pDiff) weightOnly++;
    if (differing <= 25) {
...
console.log(`synapses whose WEIGHT differs: ${differing} (of which permanence is identical: ${weightOnly}); synapses whose permanence differs: ${permanenceDiffers}`);
