...
function tempFile(): string {
  return path.join(
    mkdtempSync(path.join(tmpdir(), 'b4-checkpoint-')),
    'run.jsonl',
  );
}
 ...
 */
function checkpointedEvaluate(
  file: string,
  crashAfter: number | undefined,
  counter: { runs: number },
): Evaluate {
  const checkpoint = new Checkpoint(file);
  const key = (c: Condition, seed: bigint) =>
    `${JSON.stringify(c, (_k, v) => (typeof v === 'bigint' ? String(v) : v))}|${seed}`;
  const value = (c: Condition, seed: bigint) =>
    c.kind === 'C'
      ? (pointKey(c.point).length % 17) / 100 + Number(seed) / 1000
      : 0.1;
  return async (_stage, requests) => {
    for (const { condition, seed } of requests) {
      ...
      if (crashAfter !== undefined && counter.runs >= crashAfter)
    ...
    return (condition) =>
      new Map(
        requests
          .filter((r) =>
            key(r.condition, r.seed).startsWith(
          ...
          .map((r) => [
            r.seed,
            checkpoint.get(key(r.condition, r.seed))?.accuracy,
          ]),
      );
  };
}
...
test('a run interrupted part-way and resumed makes the same choices and never repeats a finished trial', async () => {
  ...
  const expected = await runSearch(
    new Space(),
    BUDGET,
    checkpointedEvaluate(uninterruptedFile, undefined, uninterruptedRuns),
    () => {},
  );
  ...
  await assert.rejects(
    runSearch(
      new Space(),
      BUDGET,
      checkpointedEvaluate(file, cut, runs),
      () => {},
    ),
    /simulated power cut/,
  );
  ...
  const resumed = await runSearch(
    new Space(),
    BUDGET,
    checkpointedEvaluate(file, undefined, runs),
    () => {},
  );
  assert.equal(
    runs.runs,
    uninterruptedRuns.runs,
    'the interrupted and resumed runs together must run exactly as many trials as one uninterrupted run',
  );
  assert.equal(
    pointKey(resumed.winner!.point),
    pointKey(expected.winner!.point),
  );
  assert.deepEqual(
    resumed.factorial.map((r) => r.selection?.mean),
    expected.factorial.map((r) => r.selection?.mean),
  );
  assert.equal(
