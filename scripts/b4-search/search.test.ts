...
function fakeEvaluate(landscape: Landscape) {
  ...
  const evaluate: Evaluate = async (stage, reqs) => {
    ...
    return (condition) => {
      const seeds = reqs
        .filter(
          (r) => JSON.stringify(r.condition) === JSON.stringify(condition),
        )
        .map((r) => r.seed);
      ...
      for (const seed of seeds) {
        out.set(
          seed,
          condition.kind === 'C'
            ? landscape(condition.point, condition.fixes, seed)
            : 0.1,
        );
      }
      return out;
...
function twoHills(): Landscape {
  return (point) => {
    ...
    const decoy =
      0.12 *
      Math.exp(-((a - Math.log10(0.05)) ** 2 / 0.5 + (b - 0.15) ** 2 / 0.02));
    const peak =
      0.2 *
      Math.exp(-((a - Math.log10(1)) ** 2 / 0.2 + (b - 0.45) ** 2 / 0.015));
    return (
      0.02 + Math.max(decoy, peak) + 0.0005 * Math.log2(point.stdpTauTicks)
    );
  };
}
...
async function naiveClimb(space: Space, landscape: Landscape): Promise<number> {
  ...
  for (;;) {
    const better = space
      .neighbours(point)
      .map((n) => ({ n, v: landscape(n, fixes, 1n) }))
      .sort((a, b) => b.v - a.v)[0];
...
test('the search finds the taller hill where a one-step-at-a-time climb from the middle stops on the decoy', async () => {
  ...
  assert.ok(
    naive < 0.16,
    `sanity: the naive climb should be stuck on the decoy hill, got ${naive}`,
  );
  ...
  assert.ok(
    outcome.winner.heldOut.mean > 0.15,
    `the search should reach the taller hill (well above the decoy's ~0.14), got ${outcome.winner.heldOut.mean}`,
  );
});
...
test('held-out seeds are never used before the finalists are fixed', async () => {
  ...
  const selectionStages = requests.filter(
    (r) =>
      r.stage === 'screen' ||
      r.stage === 'promote' ||
      r.stage.startsWith('refine'),
  );
  assert.ok(selectionStages.length > 0);
  assert.ok(
    selectionStages.every((r) => !heldOut.has(r.seed)),
    'screen, promote and refine must only ever use selection seeds',
  );
  ...
  assert.ok(
    firstHeldOut > lastSelection,
    'held-out seeds must first appear after every selection trial',
  );
});
...
test('confirmation seeds are used only after the winner is chosen, and only to report', async () => {
  ...
  const lastHeldOut =
    requests.length -
    1 -
    [...requests].reverse().findIndex((r) => r.stage === 'held-out finalists');
  assert.ok(
    firstConfirm > lastHeldOut,
    'confirmation seeds must first appear after the finalists are scored on held-out seeds',
  );
  const stages = new Set(
    requests.filter((r) => confirm.has(r.seed)).map((r) => r.stage),
  );
  ...
  const confirmed = new Set(
    requests
      .filter((r) => r.stage === 'confirm')
      .map((r) =>
        r.condition.kind === 'C' ? pointKey(r.condition.point) : '',
      ),
  );
  assert.deepEqual(
    confirmed,
    new Set(
      [
        outcome.winner!.point,
        ...(outcome.finalists.length > 1
          ? [outcome.winner!.runnerUp!.heldOut.point]
          : []),
      ].map(pointKey),
    ),
  );
  // The winner is the best finalist on held-out seeds, whatever confirmation says.
  const bestHeldOut = [...outcome.finalists].sort(
    (a, b) => b.heldOut!.mean - a.heldOut!.mean,
  )[0]!;
  assert.equal(
    pointKey(outcome.winner!.point),
    pointKey(bestHeldOut.selection.point),
  );
  // The extra reference runs the winner's exact config with sprouting disabled.
  const extra = requests.find(
    (r) =>
      r.stage === 'references' && r.condition.kind === 'sprout-disabled-at',
  );
  assert.ok(
    extra &&
      extra.condition.kind === 'sprout-disabled-at' &&
      pointKey(extra.condition.point) === pointKey(outcome.winner!.point),
  );
});
...
test('the search can switch a fix off when that is better, not only toggle spread', async () => {
  ...
  const landscape: Landscape = (point, fixes) =>
    0.08 +
    (fixes.timingWindow ? 0 : 0.04) +
    (fixes.elimination ? 0 : 0.03) +
    0.002 * Math.log2(point.learningRate / 0.005);
  const { evaluate } = fakeEvaluate(landscape);
  const outcome = await runSearch(space, BUDGET, evaluate, quiet);
...
test('a winner at the edge of a range drives the range outward', async () => {
  ...
  const landscape: Landscape = (point) =>
    0.05 + 0.01 * Math.log2(point.learningRate / 0.005);
  const { evaluate } = fakeEvaluate(landscape);
  const outcome = await runSearch(space, BUDGET, evaluate, quiet);
  assert.ok(outcome.winner);
  assert.ok(
    outcome.winner.point.learningRate > 2,
    `the winner should lie beyond the initial top level (2), got ${outcome.winner.point.learningRate}`,
  );
});
...
test('the factorial covers all 16 fix combinations once, at the winner, on selection seeds', async () => {
  ...
  const labels = new Set(
    outcome.factorial.map((row) => JSON.stringify(row.fixes)),
  );
  ...
  assert.ok(
    factorialRequests.every(
      (r) =>
        r.condition.kind === 'C' &&
        pointKey(r.condition.point) === pointKey(outcome.winner!.point),
    ),
  );
  const confirmRows = outcome.factorial.filter(
    (row) => row.confirm !== undefined,
  );
  assert.equal(
    confirmRows.length,
    10,
    'all off, four singles, four leave-one-outs, and all on',
  );
});
...
test('a winner that does not beat the runner-up on enough confirmation seeds is reported as not clear', async () => {
  ...
  const landscape: Landscape = (point, _fixes, seed) =>
    0.1 + (((Number(seed) * 7919 + pointKey(point).length) % 13) - 6) * 0.001;
  const { evaluate } = fakeEvaluate(landscape);
  const outcome = await runSearch(space, BUDGET, evaluate, quiet);
  assert.ok(outcome.winner);
  if (outcome.winner.runnerUp !== undefined) {
    const wins = pairedWins(
      outcome.winner.confirm!.perSeed,
      outcome.winner.runnerUp.confirm!.perSeed,
    );
...
test('a condition whose trial failed is left out of ranking and reported, not crashed on', async () => {
  ...
  const evaluate: Evaluate = async (stage, reqs) => {
    ...
    if (
      stage === 'screen' &&
      poisoned === undefined &&
      reqs[0]!.condition.kind === 'C'
    )
      poisoned = pointKey(reqs[0]!.condition.point);
    return (condition) => {
      const map = new Map(results(condition));
      if (condition.kind === 'C' && pointKey(condition.point) === poisoned)
        map.set(1n, undefined);
      return map;
    };
  };
  const outcome = await runSearch(space, BUDGET, evaluate, quiet);
...
test('hasValley: a dip below both ends separates hills; a ramp or a bump does not', () => {
  assert.equal(hasValley(0.1, 0.2, [0.05]), true, 'dips below the lower end');
  assert.equal(hasValley(0.1, 0.2, [0.12, 0.18]), false, 'a ramp');
  assert.equal(
    hasValley(0.1, 0.2, [0.25]),
    false,
    'a bump between them is one hill',
  );
  assert.equal(
    hasValley(0.1, 0.2, [0.1]),
    false,
    'level with the lower end is not a dip',
  );
  assert.equal(hasValley(0.1, 0.2, []), false, 'adjacent points are one hill');
});
...
test('a monotone landscape is one hill: only one climb, every other candidate is skipped by a hill check', async () => {
  ...
  const landscape: Landscape = (point) =>
    0.05 +
    0.01 * Math.log2(point.learningRate / 0.005) +
    0.001 * Math.log2(point.stdpTauTicks);
  const { evaluate } = fakeEvaluate(landscape);
  const outcome = await runSearch(space, BUDGET, evaluate, quiet);
...
test('the two-hill landscape is seen as two hills, and both are climbed', async () => {
  ...
  assert.ok(
    tops.some((m) => m < 0.16),
    `a finalist on the decoy hill, got ${tops}`,
  );
  assert.ok(
    tops.some((m) => m > 0.16),
    `a finalist on the taller hill, got ${tops}`,
  );
});
...
test('hill checks and climbs never exceed their budgets', async () => {
  ...
  const landscape: Landscape = (point, _fixes, seed) =>
    0.1 +
    (((Number(seed) * 7919 + pointKey(point).length * 31) % 13) - 6) * 0.001;
  const { evaluate } = fakeEvaluate(landscape);
  const outcome = await runSearch(space, BUDGET, evaluate, quiet);
  assert.ok(outcome.hillChecks.length <= BUDGET.maxHillChecks);
  assert.ok(
    outcome.finalists.length <= Math.min(BUDGET.finalists, BUDGET.refineStarts),
  );
  const rounds = new Map<number, number>();
  for (const step of outcome.refinement)
    rounds.set(step.start, (rounds.get(step.start) ?? 0) + 1);
