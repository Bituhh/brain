...
function pointAt(
  space: Space,
  pick: (levels: readonly number[]) => number,
): Point {
  return Object.fromEntries(
    PARAM_NAMES.map((name) => [name, pick(space.levelsOf(name))]),
  ) as Point;
}

const middle = (levels: readonly number[]) =>
  levels[Math.floor(levels.length / 2)]!;

test('every fix on maps each searched value to the config field it controls', () => {
  const space = new Space();
  const point: Point = {
    ...pointAt(space, middle),
    silentGate: 1,
    timingWindow: 1,
    spreadSegments: 1,
    silentElimination: 1,
  ...
  assert.deepEqual(config.silentSynapses, {
  ...
  assert.equal(
    config.tonicModulator!.channel,
    stdp.modulatorChannel,
    'the held modulator must be the channel STDP reads',
  );
});
...
test('each fix switched off removes exactly its own config, and fix 1 off keeps silence tracked', () => {
  ...
  assert.deepEqual(off.silentSynapses, {
    unsilenceWeight: point.unsilenceWeight,
    silentTransmits: true,
  });
  assert.ok(
    off.plasticity !== undefined,
    'STDP stays on in every factorial row',
  );
});
...
test('searchCondition switches each fix by its own flag in the point', () => {
  const space = new Space();
  const point = pointAt(space, middle);
  const fixesAt = (p: Point) =>
    (searchCondition(p) as Extract<Condition, { kind: 'C' }>).fixes;
  const flags = {
    silentGate: 'silentGate',
    timingWindow: 'timingWindow',
    spreadSegments: 'spread',
    silentElimination: 'elimination',
  } as const;
  for (const [param, fix] of Object.entries(flags) as [
    keyof typeof flags,
...
test('a value whose fix is off does not change the trial key, so those points share one result', () => {
  ...
  const other: Point = {
    ...point,
    maxGapTicks: space.levelsOf('maxGapTicks')[0]!,
    eliminationTicks: space.levelsOf('eliminationTicks')[0]!,
  };
  assert.notEqual(point.maxGapTicks, other.maxGapTicks);
  assert.equal(
    trialKey(searchCondition(point), 1n, 100),
    trialKey(searchCondition(other), 1n, 100),
  );
});
...
test('the references are what they claim to be', () => {
  ...
  assert.ok(
    cOff.structuralPlasticity &&
      cOff.plasticity === undefined &&
      cOff.silentSynapses === undefined,
  );
  ...
  assert.deepEqual(
    {
      ...
      structuralPlasticity: {
        ...at.structuralPlasticity!,
        minActivityStreak: search.structuralPlasticity!.minActivityStreak,
      },
    },
    search,
    'identical to the search config apart from the sprout streak',
  );
  assert.equal(
    at.structuralPlasticity!.minActivityStreak,
    disabled.structuralPlasticity!.minActivityStreak,
  );
});
...
test('trial keys are stable, differ by seed and config, and ignore property order', () => {
  ...
  assert.notEqual(
    trialKey(c, 1n, 100),
    trialKey(
      { kind: 'C', point, fixes: { ...ALL_FIXES, elimination: false } },
      1n,
      100,
    ),
  );
  assert.equal(
    canonicalJson({ b: 1, a: { d: 2n, c: 3 } }),
    canonicalJson({ a: { c: 3, d: 2n }, b: 1 }),
  );
});
...
test('configs at the bottom, middle and top of every range, and fully extended, are accepted by the native addon', () => {
  ...
  const corners = [
    pointAt(space, (l) => l[0]!),
    pointAt(space, middle),
    pointAt(space, (l) => l.at(-1)!),
  ];
  // Push every range as far as its hard bounds allow, both ways.
  let high = corners[2]!;
  let low = corners[0]!;
  for (let i = 0; i < 40; i++) {
    for (const n of space.neighbours(high))
      if (PARAM_NAMES.some((name) => n[name] > high[name]))
        high = {
          ...high,
          ...Object.fromEntries(
            PARAM_NAMES.filter((name) => n[name] > high[name]).map((name) => [
              name,
              n[name],
            ]),
          ),
        };
    for (const n of space.neighbours(low))
      if (PARAM_NAMES.some((name) => n[name] < low[name]))
        low = {
          ...low,
          ...Object.fromEntries(
            PARAM_NAMES.filter((name) => n[name] < low[name]).map((name) => [
              name,
              n[name],
            ]),
          ),
  ...
  for (const point of [...corners, high, low]) {
    for (const fixes of allFixCombinations())
      acceptedByNative({ kind: 'C', point, fixes });
  }
  acceptedByNative({ kind: 'C-off-frozen' });
  acceptedByNative({ kind: 'sprout-disabled-frozen' });
  acceptedByNative({ kind: 'A-frozen' });
  for (const point of corners)
    acceptedByNative({ kind: 'sprout-disabled-at', point });
});
