...
export function renderReport(
  ...
  lines.push('');
  lines.push(
    `Generated ${meta.generatedAt} by scripts/tune-b4-values.ts (see its header for the design). Corpus slice ${meta.corpusLength} characters. Selection seeds ${budget.fullSeeds.join(', ')} (choose everything up to the finalists); held-out seeds ${budget.heldOutSeeds.join(', ')} (choose the winner among the finalists, nothing else); confirmation seeds ${budget.confirmSeeds.join(', ')} (never used to choose -- every number reported as an estimate comes from these). Trials in the checkpoint: ${meta.trialsRun}.`,
  );
  lines.push('');
  ...
  for (const row of outcome.factorial) {
    const condition: Condition = outcome.winner
      ? { kind: 'C', point: outcome.winner.point, fixes: row.fixes }
      : { kind: 'A-frozen' };
    lines.push(
  ...
  lines.push('');
  lines.push(
    `| ${PARAM_NAMES.join(' | ')} | selection mean | held-out mean | held-out per seed |\n|${'---|'.repeat(PARAM_NAMES.length + 3)}`,
  );
