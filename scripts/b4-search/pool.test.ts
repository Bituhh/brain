...
function jobs(n: number): Job[] {
  return Array.from({ length: n }, (_, i) => ({ key: `k${i}`, label: `job${i}`, seed: BigInt(i + 1), payload: i }));
}

/** A fake trial taking `ms`, reporting progress, optionally failing or hanging. */
function fakeRunner(opts: { ms: number; failKeys?: Set<string>; hangKeys?: Set<string>; failOnceKeys?: Set<string>; onStart?: () => void; onEnd?: () => void }): TrialRunner {
  ...
  return (job, onProgress) => {
    ...
    const done = new Promise<{ accuracy: number }>((resolve, reject) => {
      rejectFn = reject;
      let step = 0;
      progressTimer = setInterval(() => onProgress(++step, 4), Math.max(1, opts.ms / 5));
      if (opts.hangKeys?.has(job.key)) return;
      timer = setTimeout(() => {
        clearInterval(progressTimer);
        opts.onEnd?.();
        if (opts.failKeys?.has(job.key) || (opts.failOnceKeys?.has(job.key) && attempt === 1)) reject(new Error("boom"));
...
const base = { log: () => {}, heartbeatMs: 1_000_000, timeoutMs: 10_000, retries: 1, stageName: "test" };
...
test("runs every job once and never exceeds the concurrency limit", async () => {
  ...
  await runJobs(jobs(9), {
    ...base,
    concurrency: 3,
    runner: fakeRunner({ ms: 20, onStart: () => { running++; peak = Math.max(peak, running); }, onEnd: () => running-- }),
...
test("a failing job is retried, then recorded as failed, while the others still finish", async () => {
  ...
  await runJobs(jobs(4), {
    ...
    log: (l) => lines.push(l),
    concurrency: 2,
    runner: fakeRunner({ ms: 10, failKeys: new Set(["k1"]), failOnceKeys: new Set(["k2"]) }),
    onResult: (r) => results.push(r),
  });
  const byKey = new Map(results.map((r) => [r.job.key, r]));
  assert.equal(byKey.get("k1")!.ok, false, "a job failing on every attempt is recorded as failed");
  assert.equal(byKey.get("k2")!.ok, true, "a job failing once succeeds on retry");
...
test("a hung job is terminated at the timeout and does not block the run", async () => {
  ...
  assert.ok(Date.now() - started < 2_000, "the run must not wait on the hung job");
});
...
test("the heartbeat keeps logging while trials run, showing progress", async () => {
  ...
  assert.ok(beats.length >= 3, `expected several heartbeats during a 150 ms trial, got ${beats.length}`);
  assert.ok(beats.some((l) => /job\d s\d+ \d+%/.test(l)), "heartbeats must show each running trial's progress");
  assert.ok(lines.some((l) => l.startsWith("[done 2/2]")));
});

test("no heartbeat keeps running after the run finishes", async () => {
  const lines: string[] = [];
  await runJobs(jobs(1), { ...base, log: (l) => lines.push(l), heartbeatMs: 10, concurrency: 1, runner: fakeRunner({ ms: 5 }), onResult: () => {} });
