...
export type Point = Readonly<Record<ParamName, number>>;
...
export function pointKey(point: Point): string {
  return PARAM_NAMES.map((name) => `${name}=${point[name]}`).join(",");
}

export class Space {
  readonly #specs: ReadonlyMap<ParamName, ParamSpec>;
  readonly #levels = new Map<ParamName, number[]>();

  constructor(specs: readonly ParamSpec[] = B4_PARAM_SPECS) {
    this.#specs = new Map(specs.map((spec) => [spec.name, spec]));
    for (const name of PARAM_NAMES) {
      const spec = this.#specs.get(name);
      if (spec === undefined) throw new Error(`missing parameter spec: ${name}`);
      const sorted = [...spec.initial].sort((a, b) => a - b);
      if (sorted.length < 2) throw new Error(`${name} needs at least two levels`);
      this.#levels.set(name, sorted);
    }
  }

  levelsOf(name: ParamName): readonly number[] {
    return this.#levels.get(name)!;
  }

  indexOf(name: ParamName, value: number): number {
   ...
   */
  between(a: Point, b: Point, fraction: number): Point {
    return Object.fromEntries(PARAM_NAMES.map((name) => {
   ...
   */
  sample(n: number, seed: number): Point[] {
    const random = prng(seed);
    const columns = new Map<ParamName, number[]>();
    for (const name of PARAM_NAMES) {
      const levels = this.levelsOf(name);
    ...
    for (let row = 0; row < n; row++) {
      const point = Object.fromEntries(PARAM_NAMES.map((name) => [name, columns.get(name)![row]!])) as Point;
    ...
    while (points.length < n && guard++ < n * 100) {
      const point = Object.fromEntries(PARAM_NAMES.map((name) => {
        const levels = this.levelsOf(name);
