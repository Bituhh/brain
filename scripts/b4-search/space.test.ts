...
test('the sample returns n distinct points, every value a level of its parameter', () => {
  ...
  for (const point of points) {
    for (const name of PARAM_NAMES)
      assert.ok(
        space.levelsOf(name).includes(point[name]),
        `${name}=${point[name]}`,
      );
  }
});
...
test('the sample covers every level of every parameter (the whole map, not one corner)', () => {
  ...
  for (const name of PARAM_NAMES) {
    ...
    assert.equal(
      used.size,
      space.levelsOf(name).length,
      `${name} should use all ${space.levelsOf(name).length} levels, used ${[...used].join(',')}`,
    );
  }
});
...
test('neighbours step one level either way on each parameter, and toggle the boolean', () => {
  ...
  const point = Object.fromEntries(
    PARAM_NAMES.map((name) => [name, space.levelsOf(name)[1]!]),
  ) as Point;
  const neighbours = space.neighbours(point);
  for (const name of PARAM_NAMES) {
    const levels = space.levelsOf(name);
    const changed = neighbours
      .filter((n) => n[name] !== point[name])
      .map((n) => n[name]);
    if (levels.length === 2) {
      ...
      assert.deepEqual(
        ...
        [levels[0], levels[2]],
        `${name} should step to both adjacent levels`,
      );
    }
  }
  for (const n of neighbours) {
    ...
    assert.equal(
      differing.length,
      1,
      'each neighbour differs from the point in exactly one parameter',
    );
  }
});
...
test('stepping past the top level extends the range instead of stopping at the edge', () => {
  ...
  const top = Object.fromEntries(
    PARAM_NAMES.map((name) => [name, space.levelsOf(name).at(-1)!]),
  ) as Point;
...
test("extension stops at each parameter's hard bound", () => {
  ...
  for (let i = 0; i < 20; i++) {
    const up = space
      .neighbours(point)
      .find((n) => n.maxGapTicks > point.maxGapTicks);
    if (up === undefined) break;
    point = up;
  }
  assert.ok(
    point.maxGapTicks <= 200,
    `maxGapTicks must stay within one sweep interval, got ${point.maxGapTicks}`,
  );
  let low = Object.fromEntries(
    PARAM_NAMES.map((name) => [name, space.levelsOf(name)[0]!]),
  ) as Point;
  for (let i = 0; i < 50; i++) {
    const down = space
      .neighbours(low)
      .find((n) => n.unsilenceWeight < low.unsilenceWeight);
    if (down === undefined) break;
    low = down;
  }
  assert.ok(
    low.unsilenceWeight > 0.05,
    `unsilenceWeight must stay above sproutWeight (0.05), got ${low.unsilenceWeight}`,
  );
});
...
test('every level of every parameter is a value the native config accepts in principle', () => {
  ...
  assert.ok(
    space.levelsOf('maxGapTicks').every((g) => Number.isInteger(g) && g >= 1),
  );
});
...
test("between walks every parameter's levels at once and rounds to a level", () => {
  ...
  assert.deepEqual(space.between(low, lrOnly, 0.5), {
    ...low,
    learningRate: space.levelsOf('learningRate')[4]!,
  });
});
