...
export function workerRunner(workerPath: string, corpus: string): TrialRunner {
  return (job, onProgress) => {
    const worker = new Worker(workerPath, {
      workerData: { corpus, seed: job.seed, config: job.payload },
    });
    let settled = false;
    const done = new Promise<TrialOutput>((resolve, reject) => {
      worker.on('message', (message: WorkerMessage) => {
        if (message.type === 'progress') {
          ...
          const consolidation = message.consolidationStats as
            ConsolidationStats | undefined;
          resolve({
            accuracy: message.accuracy,
            ...(stats !== undefined && { structuralStats: stats }),
      ...
      worker.once('exit', (code) => {
        if (!settled) {
          ...
          reject(
            new Error(
              `worker exited with code ${code} before reporting a result`,
            ),
          );
        }
      });
...
export interface EvaluatorOptions<TCondition = Condition> extends Omit<
  PoolOptions,
  'onResult' | 'stageName'
> {
...
export function makeEvaluate<TCondition = Condition>(
  options: EvaluatorOptions<TCondition>,
): Evaluate<TCondition> {
  const { checkpoint, corpusLength } = options;
  const codec =
    options.codec ??
    (defaultB4Codec() as unknown as ConditionCodec<TCondition>);
  return async (stage, requests) => {
    ...
    for (const { condition, seed } of requests) {
      ...
      jobs.push({
        key,
        label: codec.conditionLabel(condition),
        seed,
        payload: codec.toConfig(condition),
      });
    }
    options.log(
      `[stage] ${stage}: ${queued.size} distinct trials, ${alreadyDone} already in the checkpoint, ${jobs.length} to run`,
    );
    ...
    return (condition: TCondition): Evaluation => {
      ...
      for (const seed of seedsByConfig.get(
        codec.configKey(condition, corpusLength),
      ) ?? []) {
        const record = checkpoint.get(
          codec.trialKey(condition, seed, corpusLength),
        );
