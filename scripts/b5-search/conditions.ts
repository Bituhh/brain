...
import { fixesOf, stdpConfig as b4StdpConfig, TONIC_CHANNEL, TONIC_LEVEL, type Fixes } from "../b4-search/conditions.ts";
...
export function learningTargetOf(point: Point<B5ParamName>): "permanence" | "weight" | "both" {
  return point.predictiveLearningTarget === 0 ? "permanence" : point.predictiveLearningTarget === 1 ? "weight" : "both";
}
...
export function voteReferenceWeightOf(point: Point<B5ParamName>): number | undefined {
  return point.voteReferenceWeight === 0 ? undefined : point.voteReferenceWeight;
}

function homeostaticScalingOf(point: Point<B5ParamName>): CharPredictionConfig["homeostaticScaling"] {
  ...
  return point.homeostaticScaling === 1 ? { targetTotalWeight: 6.0, intervalTicks: 200 } : undefined;
}
...
export type Condition =
  ...
  | { readonly kind: "C"; readonly point: Point<B5ParamName>; readonly fixes: Fixes }
