...
import { conditionLabel, configKey, searchCondition, toConfig, trialKey, type Condition } from "./conditions.ts";
...
function pointAt(winnerPoint: Point<B5ParamName>, combo: FactorialCombo): Point<B5ParamName> {
  return {
    ...winnerPoint,
    ...
    voteReferenceWeight: combo.voteMode === "count" ? 0 : winnerPoint.voteReferenceWeight === 0 ? 0.65 : winnerPoint.voteReferenceWeight,
    silentGate: combo.silentGate ? 1 : 0,
    predictiveLearningTarget: learningTargetLevel(combo.learningTarget),
  };
}

export function toFactorialCondition(winnerPoint: Point<B5ParamName>, combo: FactorialCombo): Condition {
...
export function defaultB5Hooks(): SearchHooks<B5ParamName, Condition, FactorialCombo> {
  return {
    ...
    references: (winnerPoint) => [
      ...
      ...(winnerPoint !== undefined
        ? [
            { name: "condition A, at the winner's own vote settings", condition: { kind: "A-at" as const, point: winnerPoint } },
            { name: "the winner's exact config with sprouting disabled", condition: { kind: "sprout-disabled-at" as const, point: winnerPoint } },
          ]
        : []),
      { name: "B4's count-mode winner (char-prediction.slow.test.ts)", condition: { kind: "b4-count-winner" } },
    ],
  };
}
