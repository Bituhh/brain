...
import { conditionLabel, searchCondition, voteReferenceWeightOf, type Condition } from "./conditions.ts";
...
function pointCells(point: Point<B5ParamName>): string {
  return B5_PARAM_NAMES.map((name) => (name === "voteReferenceWeight" ? String(voteReferenceWeightOf(point) ?? "count") : String(point[name]))).join(" | ");
}
...
function scoredRow(s: Scored<B5ParamName>, seeds: readonly bigint[], lookup: Lookup): string {
...
export function renderReport(
  ...
  budget: Budget,
  lookup: Lookup,
  meta: { readonly generatedAt: string; readonly corpusLength: number; readonly trialsRun: number },
): string {
  const lines: string[] = [];
  lines.push("# B5 value search -- results");
  lines.push("");
  lines.push(`Generated ${meta.generatedAt} by scripts/tune-b5-values.ts (see its header for the design). Corpus slice ${meta.corpusLength} characters. Selection seeds ${budget.fullSeeds.join(", ")}; held-out seeds ${budget.heldOutSeeds.join(", ")}; confirmation seeds ${budget.confirmSeeds.join(", ")} (never used to choose). Trials in the checkpoint: ${meta.trialsRun}.`);
  lines.push("");
  ...
  if (outcome.winner === undefined) {
    ...
    lines.push(`| ${B5_PARAM_NAMES.join(" | ")} |\n|${"---|".repeat(B5_PARAM_NAMES.length)}\n| ${pointCells(w.point)} |`);
    lines.push("");
    ...
    if (w.runnerUp !== undefined) {
      lines.push(`Runner-up: confirmation mean ${pct(w.runnerUp.confirm?.mean)}, held-out ${pct(w.runnerUp.heldOut.mean)}. ${w.clear ? `Clear win: better on at least ${budget.clearWinSeeds} of ${budget.confirmSeeds.length} confirmation seeds.` : `**Not a clear win** -- treat the winner and runner-up as tied.`}`);
    }
  }
  lines.push("");

  lines.push("## Factorial at the winner: vote mode x silent gate x learning target (Requirement 9.4)");
  lines.push("");
  lines.push("| combo | selection mean | per seed | confirmation mean | confirmation per seed | sprouted / unsilenced / eliminated / silent now (selection mean) |");
  lines.push("|---|---|---|---|---|---|");
  for (const row of outcome.factorial) {
    const condition: Condition | undefined = outcome.winner ? toFactorialCondition(outcome.winner.point, row.fixes) : undefined;
    lines.push(
  ...
  lines.push("");
  lines.push(`| ${B5_PARAM_NAMES.join(" | ")} | selection mean | held-out mean | held-out per seed |\n|${"---|".repeat(B5_PARAM_NAMES.length + 3)}`);
...
export interface ChosenValues {
  ...
  readonly factorial: readonly { readonly combo: FactorialCombo; readonly selectionMean: number | undefined; readonly confirmMean: number | undefined }[];
  readonly references: readonly { readonly name: string; readonly confirmMean: number | undefined }[];
}

export function chosenValues(outcome: SearchOutcome<B5ParamName, FactorialCombo>, generatedAt: string): ChosenValues {
  return {
    generatedAt,
    ...
    factorial: outcome.factorial.map((row) => ({ combo: row.fixes, selectionMean: row.selection?.mean, confirmMean: row.confirm?.mean })),
