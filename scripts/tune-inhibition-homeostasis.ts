...
import { runCharPredictionTrials, assessMilestone, DEFAULT_CONFIG, NETWORK_DENSITY, type CharPredictionConfig } from "../packages/io/src/milestone/charPrediction.ts";

const logPath = fileURLToPath(new URL("./tune-inhibition-homeostasis.results.md", import.meta.url));
writeFileSync(
  logPath,
...
function logTrial(label: string, seedCount: number, meanAccuracy: number, perSeed: readonly number[], note: string): void {
  const range = perSeed.length > 0 ? `${(Math.min(...perSeed) * 100).toFixed(2)}%–${(Math.max(...perSeed) * 100).toFixed(2)}%` : "—";
  appendFileSync(logPath, `| ${label} | ${seedCount} | ${(meanAccuracy * 100).toFixed(2)}% | ${range} | ${note} |\n`);
}
...
const BASE = { smoothing: 0.9, adjustmentRate: 4.0, minK: 1, intervalTicks: 200 };
...
function measure(label: string, targetRate: number | undefined, seeds: readonly bigint[], note: string): number {
  const trials = runCharPredictionTrials(corpus, seeds, configFor(targetRate));
  const assessment = assessMilestone(trials);
  logTrial(label, seeds.length, assessment.meanNetworkAccuracy, trials.map((t) => t.networkAccuracy), note);
  return assessment.meanNetworkAccuracy;
}

console.log(`Baseline (disabled) vs. targetRate grid ${CANDIDATE_TARGET_RATES.join(", ")} (network density = ${NETWORK_DENSITY}), ${SEARCH_SEEDS.length} search seeds each.\n`);

const t0 = Date.now();
const baselineScore = measure("disabled", undefined, SEARCH_SEEDS, "baseline (mechanism disabled)");
console.log(`  baseline: mean network accuracy=${(baselineScore * 100).toFixed(2)}% (${Date.now() - t0}ms)`);

let best: { targetRate: number; score: number } | undefined;
for (const targetRate of CANDIDATE_TARGET_RATES) {
  const tTrial = Date.now();
  const score = measure(targetRate.toFixed(4), targetRate, SEARCH_SEEDS, "search");
  console.log(`  targetRate=${targetRate.toFixed(4)}: mean network accuracy=${(score * 100).toFixed(2)}% (${Date.now() - tTrial}ms)`);
...
const confirmedDisabled = assessMilestone(runCharPredictionTrials(corpus, CONFIRM_SEEDS, configFor(undefined)));
logTrial("disabled", CONFIRM_SEEDS.length, confirmedDisabled.meanNetworkAccuracy, [], "**confirmed (official protocol), baseline**");
console.log(`  disabled: mean network accuracy=${(confirmedDisabled.meanNetworkAccuracy * 100).toFixed(2)}%`);

if (winner !== undefined) {
  const confirmedWinner = assessMilestone(runCharPredictionTrials(corpus, CONFIRM_SEEDS, configFor(winner.targetRate)));
  logTrial(winner.targetRate.toFixed(4), CONFIRM_SEEDS.length, confirmedWinner.meanNetworkAccuracy, [], "**confirmed (official protocol), best candidate**");
  console.log(`  targetRate=${winner.targetRate.toFixed(4)}: mean network accuracy=${(confirmedWinner.meanNetworkAccuracy * 100).toFixed(2)}%`);
  console.log(
