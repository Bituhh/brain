...
function report(name: string, config: typeof winner): void {
  runCharPredictionTrial(corpus, SEED, config, undefined, (sim: Simulation) => {
    ...
    const bins = [
      0, 0.05, 0.1, 0.2, 0.3, 0.35, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.999999,
      1.0000001,
    ];
    console.log(`\n=== ${name} (${CHARS} chars, seed ${SEED}) ===`);
    console.log(
      `occupied synapses ${n}; connection threshold ${threshold}; prune floor ${PRUNE_FLOOR}; sprout birth permanence ${SPROUT_PERMANENCE}`,
    );
    console.log(
      `gated events over the whole run: correct (reinforce) ${totals.correct}, falsePositive (punish) ${totals.falsePositive}, ratio ${(totals.correct / Math.max(1, totals.falsePositive)).toFixed(1)}:1`,
    );
    console.log(
      `reinforce ${REINFORCE} x g and punish ${PUNISH} x g per event, swept g up to ${G_MAX} => one punish moves permanence by at most ${punishStep.toFixed(3)}`,
    );
    console.log(
      `\npermanence == 1.0 (at the clamp)          ${pct(count((p) => p === 1))}`,
    );
    console.log(
      `permanence == 0.0                          ${pct(count((p) => p === 0))}`,
    );
    console.log(
      `connected (>= threshold)                   ${pct(count((p) => p >= threshold))}`,
    );
    console.log(
      `  ...within one punish step above it       ${pct(count((p) => p >= threshold && p < threshold + punishStep))}   <- could drop out on ONE write at the top of the sweep`,
    );
    console.log(
      `  ...within two punish steps above it      ${pct(count((p) => p >= threshold && p < threshold + 2 * punishStep))}`,
    );
    console.log(
      `sub-threshold (< threshold)                ${pct(count((p) => p < threshold))}`,
    );
    console.log(
      `  ...within one reinforce step below it    ${pct(count((p) => p < threshold && p >= threshold - REINFORCE * G_MAX))}   <- could connect on ONE write`,
    );
    console.log(
      `near the prune floor (< floor + one punish) ${pct(count((p) => p < PRUNE_FLOOR + punishStep))}`,
    );
    console.log(`\nhistogram of permanence (lower edge inclusive):`);
    for (let b = 0; b + 1 < bins.length; b++) {
      const k = count((p) => p >= bins[b]! && p < bins[b + 1]!);
      console.log(
        `  [${bins[b]!.toFixed(3)}, ${bins[b + 1]!.toFixed(3)})  ${String(k).padStart(7)}  ${'#'.repeat(Math.round((k / n) * 60))}`,
      );
    }
    ...
    for (const [v, k] of [...byValue.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10))
      console.log(`  ${v.toFixed(6)}  x ${k}`);
  });
}

report(
  "B5's winner, no reward signal (dopamine-gated path OFF; scale 1.0)",
  winner,
);
report("B5's winner + rewardSignal, dopamine held at 1.0 (gated path ON)", {
  ...
  rewardPredictionError: {
    tauEvents: 200,
    drive: { channel: 0, baseline: 1.0, gain: 0.0, maxLevel: 4.0 },
  },
});
