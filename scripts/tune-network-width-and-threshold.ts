...
const logPath = fileURLToPath(
  new URL('./tune-network-width-and-threshold.results.md', import.meta.url),
);
writeFileSync(
  logPath,
...
if (BASE_HOMEOSTASIS === undefined) {
  throw new Error(
    "DEFAULT_CONFIG.segmentThresholdHomeostasis must be set as this search's starting point.",
  );
}
...
function searchWidth(
  round: number,
  fixedTargetRate: number,
  startWidth: number,
): { width: number; score: number } {
  let best = {
    width: startWidth,
    score: measure(
      round,
  ...
  while (step >= WIDTH_MIN_STEP) {
    ...
    if (better !== undefined) {
      console.log(
        `    -> width improved: ${best.width} (${(best.score * 100).toFixed(2)}%) -> ${better.width} (${(better.score * 100).toFixed(2)}%)`,
      );
      best = better;
    } else {
...
function searchRate(
  ...
  while (step >= RATE_MIN_STEP) {
    ...
    if (better !== undefined) {
      console.log(
        `    -> targetRate improved: ${best.rate.toFixed(4)} (${(best.score * 100).toFixed(2)}%) -> ${better.rate.toFixed(4)} (${(better.score * 100).toFixed(2)}%)`,
      );
      best = better;
    } else {
      step /= 2;
      console.log(
        `    -> targetRate: no improvement, halving step to ${step.toFixed(4)}`,
      );
    }
  }
  return best;
...
console.log(
  `Starting joint search from width=${DEFAULT_CONFIG.width}, targetRate=${BASE_HOMEOSTASIS.targetRate}.`,
);
console.log(
  `Width bounds [${WIDTH_LOWER_BOUND}, ${WIDTH_UPPER_BOUND}], targetRate bounds [${RATE_LOWER_BOUND}, ${RATE_UPPER_BOUND}].\n`,
);
...
for (; round <= MAX_ROUNDS; round++) {
  ...
  if (!widthChanged && !rateChanged) {
    console.log(
      `Round ${round} changed neither axis -- coordinate descent has converged.`,
    );
    break;
  }
}
...
console.log(
  `Best found: width=${currentWidth}, targetRate=${currentRate.toFixed(4)}`,
);
...
const confirmTrials = runCharPredictionTrials(
  corpus,
  CONFIRM_SEEDS,
  configFor(currentWidth, currentRate),
);
