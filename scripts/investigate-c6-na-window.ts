...
const workers = Math.max(1, Math.min(Number(process.env.C6_WORKERS ?? 12), cpus().length));
...
const coupling = { tauFastTicks: 100, tauSlowTicks: 2000, unexpected: { channel: NORADRENALINE, baseline: 1.0, gain: 1.0, maxLevel: 4.0 } };
const windowMap = (reference: number, gain: number): LevelMapConfig => ({ channel: NORADRENALINE, reference, gain, min: MAP_MIN, max: MAP_MAX });
const joint = (map: LevelMapConfig) => ({ tauPlus: map, tauMinus: map, windowTicks: map });

/** Coupling on, the window map on noradrenaline, observed. */
const coupled = (reference: number, gain: number): CharPredictionConfig => ({
  ...winner,
  plasticity: { ...plasticity, stdpModulation: joint(windowMap(reference, gain)), observeStdpModulation: true },
...
const held = (level: number, gain: number): CharPredictionConfig => ({
  ...
  plasticity: {
    ...plasticity,
    modulatorTauTicks: plasticity.modulatorTauTicks.map((tau, channel) => (channel === NORADRENALINE ? 1.0e30 : tau)),
...
const phase1: Job[] = [
  ...SEEDS.map((seed) => ({ key: keyOf(measureConfig, seed), label: "M: coupling on, map gain 0 (measures reference)", seed, config: measureConfig })),
  ...HASH_SEEDS.map((seed) => ({ key: keyOf(freshConfig, seed), label: "F: B5's winner, fresh (hashes for the controls)", seed, config: freshConfig })),
];
log(`=== investigate-c6-na-window: ${workers} workers, ${CORPUS_LENGTH} characters, seeds ${SEEDS.join(", ")} ===`);
...
if (measured.some((r) => r?.stdpModulation == null)) throw new Error("phase 1 incomplete or unobserved -- re-run to resume; not proceeding to phase 2 without a measured reference");
const restLevels = measured.map((r) => r!.stdpModulation!.minLevel[NORADRENALINE]!);
const reference = Math.min(...restLevels);
log(`measured resting level (min over ten M rows of the level a pairing read): ${reference} (per seed: ${restLevels.join(", ")})`);
...
const phase2: Job[] = [
  ...HASH_SEEDS.map((seed) => ({ key: keyOf(heldConfig, seed), label: `X: hook at gain ${MAP_GAINS[0]}, noradrenaline held at reference`, seed, config: heldConfig })),
  ...MAP_GAINS.flatMap((gain) => SEEDS.map((seed) => ({ key: keyOf(gainConfig(gain), seed), label: `W: map gain ${gain}`, seed, config: gainConfig(gain) }))),
];
await runAll(phase2);
...
out.push(`# PLAN.md C6 -- noradrenaline widens the STDP window: the pre-registered VAL-4 confirmation`);
out.push(``);
out.push(`Generated ${new Date().toISOString()}. ${CORPUS_LENGTH} characters per trial, B5's winner as the base, accuracy is the harness's 2,000-character`);
out.push(`sliding window at the end of the run. Every choice below was written into the script header before any trial ran.`);

out.push(``);
out.push(`## 1. B5's figures, reproduced, and the exactness controls -- all must PASS before section 3 is read`);
out.push(``);
const refAcc = (seeds: readonly bigint[]) => seeds.map((s) => referenceRecord(s)!.accuracy!);
out.push(`Reference (checkpointed B5 winner): seeds 1-5 mean **${pct(mean(refAcc(SELECTION)))}** (B5: 20.36%), seeds 11-15 **${pct(mean(refAcc(CONFIRMATION)))}** (B5: 19.05%),`);
out.push(`seeds 1 / 2 / 3 ${refAcc([1n, 2n, 3n]).map(pct).join(" / ")} (B5: 19.85 / 20.50 / 21.10%).`);
out.push(``);
let pass = true;
for (const seed of SEEDS) {
  ...
  pass &&= ok;
  out.push(`- M seed ${seed} (coupling on, hook live at gain 0) vs checkpointed reference, accuracy + structural totals: **${ok ? "PASS" : "FAIL"}**`);
}
for (const seed of HASH_SEEDS) {
  ...
  if (f) {
    ...
    pass &&= ok;
    out.push(`- F seed ${seed} (fresh B5 winner) vs checkpointed reference: **${ok ? "PASS" : "FAIL"}**`);
  }
  if (f && m) {
    const ok = sameBits(m, f);
    pass &&= ok;
    out.push(`- M seed ${seed} vs F, bit for bit (topology, permanence, weight hashes): **${ok ? "PASS" : "FAIL"}**`);
  }
  if (f && x) {
    const ok = sameBits(x, f);
    pass &&= ok;
    out.push(`- X seed ${seed} (hook at gain ${MAP_GAINS[0]}, level held exactly at the reference) vs F, bit for bit: **${ok ? "PASS" : "FAIL"}**`);
  }
}
out.push(``);
out.push(pass ? `All controls PASS.` : `**A CONTROL FAILED -- section 3 must not be read until this is explained.**`);

out.push(``);
out.push(`## 2. The measured reference`);
out.push(``);
out.push(`The level of noradrenaline a pairing actually read, over the whole run (\`stdpModulationStats()\`, M rows):`);
out.push(``);
out.push(`| seed | min level read (rest) | max level read | max excursion above rest |`);
out.push(`| --- | --- | --- | --- |`);
...
out.push(``);
out.push(`**reference = ${reference}** (the minimum over the ten, by the pre-registered rule; ${new Set(restLevels).size === 1 ? "all ten agree to the bit" : `${new Set(restLevels).size} distinct values`}).`);

out.push(``);
out.push(`## 3. The confirmation`);
out.push(``);
out.push(`Paired against the checkpointed reference, seed by seed. Threshold (pre-registered): an effect only if |mean change| >= ${THRESHOLD_POINTS.toFixed(1)} point`);
out.push(`with the same sign on both seed sets. "Always guess space" is **${pct(ALWAYS_SPACE)}**.`);
out.push(``);
out.push(`| map gain | seed | accuracy | reference | change (points) | pairings | scale != 1 | admitted by widening | max scale |`);
out.push(`| --- | --- | --- | --- | --- | --- | --- | --- | --- |`);
...
for (const gain of MAP_GAINS) {
  ...
  const setMean = (seeds: readonly bigint[]) => {
    const v = seeds.map((s) => deltas.get(s)).filter((d): d is number => d !== undefined);
  ...
  const accs = (seeds: readonly bigint[]) => seeds.map((s) => done.get(keyOf(gainConfig(gain), s))?.accuracy).filter((a): a is number => a !== undefined);
  out.push(`| **${gain}** | **1-5 mean** | **${pct(mean(accs(SELECTION)))}** | ${pct(mean(refAcc(SELECTION)))} | **${pts(sel)}** | | | | |`);
  out.push(`| **${gain}** | **11-15 mean** | **${pct(mean(accs(CONFIRMATION)))}** | ${pct(mean(refAcc(CONFIRMATION)))} | **${pts(conf)}** | | | | |`);
  verdicts.push(
...
out.push(``);
out.push(`"Pairings" is every STDP kernel evaluation over the run; "scale != 1" the share at which the level moved the curve at all; "admitted by`);
out.push(`widening" the share that counted only because the window was wider than configured.`);
out.push(``);
out.push(`## Verdict`);
out.push(``);
out.push(...verdicts);
