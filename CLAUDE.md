...
code** — see "Evidence for changes" and "Recording tests, experiments and tuning"
below.
...
4. **`docs/`** — everything not directly in README's spec: evidence, decisions,
   open questions, findings, build history. See the routing table below for
   what goes where.
...
neuromodulation, anything that alters simulated *behaviour* — needs a cited
...
- **Mechanism changes.** Cite the paper(s) the design is drawn from. Add or
  ...
  the paper *actually found*, including where our implementation diverges from
...
- **Engineering changes** (performance, FFI, build tooling, test infra, refactors
  with no behavioural effect) — a rationale note in
...
2. The **raw data** — full tables, per-seed results, sweep output — goes in a
   file under [`docs/appendix/`](docs/appendix/), referenced from the finding
   by a relative link (`see docs/appendix/<file>.md`), not pasted inline. This
...
to `.claude/HANDOFF.md`, is timestamped **`[YYYY-MM-DD HH:MM ±ZZZZ]`** (ISO 8601,
sortable) — e.g. `[2026-09-22 14:30 +0100]`. This matches the dating convention
already used throughout the project (`2026-09-20 23:05 +0100`-style timestamps
predate this rule and are not being reformatted); it is now mandatory rather
than incidental, specifically so the sequence of decisions and findings can be
reconstructed later for the paper.
