...
test('a view reflects Rust-side mutation with no copy (Requirement 2.1, 2.3)', () => {
  ...
  assert.equal(
    view.membrane[0],
    42.5,
    'view must reflect the Rust-side mutation with no re-fetch',
  );
});
...
test('epoch does not change when no growth occurs', () => {
  ...
  assert.equal(
    brain.currentEpoch(),
    before,
    'mutation alone must not bump the epoch',
  );
});
...
test('membrane view is cached per epoch, not re-minted on every access', () => {
  ...
  assert.equal(
    first,
    second,
    'same epoch must return the same cached typed array instance',
  );
});
...
test('liveCount tracks allocations and epoch tracks growth independently', () => {
  ...
  assert.equal(
    brain.currentEpoch(),
    2,
    'two appends, no reuse -> epoch grows with each',
  );
});
...
test('Simulation: a spike is delivered at exactly tick + delay (Requirement 5.4)', () => {
  ...
  for (let tick = 1; tick < 5; tick++) {
    ...
    assert.equal(
      sim.membraneAt(b),
      0,
      `b must be untouched before tick 5, currently at tick ${tick}`,
    );
  }
  sim.step(); // tick 5
...
test('Simulation.connect reports budget exhaustion instead of throwing (Requirement 11.3)', () => {
  ...
  assert.equal(
    sim.connect(a, c, 0, 1, 0.9),
    undefined,
    'capacity-1 block must reject a second synapse',
  );
});
...
test('Simulation: snapshot and restore round-trip a running simulation (Requirement 16.11)', () => {
  ...
  try {
    ...
    assert.equal(
      ...
      tickBeforeSave,
      'restored simulation must resume at the same tick',
    );
    assert.equal(
      restored.membraneAt(b),
      membraneBeforeSave,
      "restored simulation's neuron state must match exactly",
    );
    ...
    for (let tick = 0; tick < 30; tick++) {
      ...
      assert.deepEqual(
        spikedRestored,
        spikedOriginal,
        `tick ${tick}: restored and original must spike identically`,
      );
    }
  } finally {
...
test('Simulation: inhibition limits spikes to k winners per neighbourhood (Requirement 7.1)', () => {
  const sim = Simulation.create(
    { tauMTicks: 5, vRest: 0, vReset: 0, refractoryTicks: 2 },
    {
      ...
      inhibition: { neighbourhoodSize: 10, k: 1 },
    },
  );
  ...
  assert.deepEqual(
    spiked,
    [indices[1]],
    'only the highest-margin candidate should win a k=1 neighbourhood',
  );
});
...
test("Simulation: threadCount > 1 reproduces threadCount 1's spike sequence exactly, through the real FFI boundary (Phase 4, Requirement 8)", () => {
  ...
  assert.deepEqual(
    partitioned,
    single,
    "threadCount 4 must reproduce threadCount 1's spike sequence exactly, tick by tick",
  );
});
...
test('Simulation.buildColumns produces correct, non-overlapping neuron-index ranges (Requirement 8.1)', () => {
  ...
  const handles = sim.buildColumns(1n, [
    columnConfig({ neuronCount: 4, baseY: 0 }),
    columnConfig({ neuronCount: 6, baseY: 10 }),
  ]);

  assert.equal(handles.length, 2);
  assert.deepEqual(
    [handles[0]!.id, handles[0]!.start, handles[0]!.end],
    [0, 0, 4],
  );
  assert.deepEqual(
    [handles[1]!.id, handles[1]!.start, handles[1]!.end],
    [1, 4, 10],
  );
});
...
test('Simulation.buildColumns wires lateral voting only between named columns (Requirement 8.1, NET-5)', () => {
  ...
  const votingPolicy: VotingGroupConfig['policy'] = {
    p0: 1.0,
    lengthScale: 1000.0,
    delayMin: 1,
    delayMax: 1,
  ...
  const handles = sim.buildColumns(
    1n,
    [
      columnConfig({ neuronCount: 2, baseY: 0 }),
      columnConfig({ neuronCount: 2, baseY: 5 }),
    ],
...
test('Simulation.buildColumns wires gating suppression only between named columns (Requirement 3, NET-13)', () => {
  ...
  const gatingPolicy: GatingGroupConfig['policy'] = {
    p0: 1.0,
    lengthScale: 1000.0,
    delayMin: 1,
    delayMax: 1,
    initialPermanence: 0.95,
  };
  const TICKS = 60;
  ...
  function runScenario(wireGating: boolean): number {
    ...
    const columnA = columnConfig({
      neuronCount: 6,
      excitatoryFraction: 0.0,
      baseY: 0,
    });
    const columnB = columnConfig({
      neuronCount: 3,
      excitatoryFraction: 1.0,
      baseY: 1000,
    });
    const gatingGroups: GatingGroupConfig[] = wireGating
      ? [{ columnIds: [0, 1], policy: gatingPolicy }]
      : [];
  ...
  assert.ok(
    withoutGating > 0,
    'column B must fire reliably on its own when gating is not wired',
  );
  assert.ok(
...
test('Simulation.buildColumns is additive: a flat (no build_columns) network is unaffected (Requirement 8.2)', () => {
  ...
  assert.deepEqual(
    buildAndRun(4, neuronCount),
    buildAndRun(),
    'a network that never calls buildColumns must be unaffected by its existence',
  );
});
...
test("Simulation.buildColumns: threadCount > 1 reproduces threadCount 1's spike sequence exactly for a column-built network (Requirement 8, PartitionPlan::contiguous)", () => {
  ...
  function buildAndRun(threadCount?: number): number[][] {
    ...
    const columns = Array.from({ length: columnCount }, (_, i) =>
      columnConfig({ neuronCount: columnSize, baseY: i * 100, k: 1 }),
    );
  ...
  assert.deepEqual(
    buildAndRun(4),
    buildAndRun(),
    "threadCount 4 (PartitionPlan::contiguous) must reproduce threadCount 1's spike sequence exactly for a column-built network",
  );
});
...
test('Simulation.buildColumns: column membership round-trips through snapshot/restore exactly (Requirement 8.5)', () => {
  ...
  try {
    ...
    const original = Simulation.create(lif, options);
    const handles = original.buildColumns(9n, [
      columnConfig({ neuronCount: 3, baseY: 0 }),
      columnConfig({ neuronCount: 5, baseY: 20 }),
    ]);
    ...
    for (let tick = 0; tick < 20; tick++) {
      ...
      assert.deepEqual(
        restored.step(),
        original.step(),
        `tick ${tick}: restored column-built network must spike identically to the original`,
      );
    }
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('Simulation.membraneView reflects Rust-side mutation with no copy, mirroring Brain.views() (Requirement 8.4)', () => {
  ...
  assert.equal(
    view[0],
    42.5,
    'membraneView must reflect the Rust-side mutation with no re-fetch',
  );
});
...
test('Simulation.predictiveView is a bulk zero-copy view, independent of membraneView (Requirement 8.4)', () => {
  ...
  assert.ok(
    predictiveView[b]! > 0,
    "predictiveView must reflect b's dendritic depolarisation with no re-fetch",
  );
});
...
test("SegmentsConfig.voteReferenceWeight: weighted mode changes whether a weak synapse's coincidence depolarises the target (Requirement 1, 10's FFI layer)", () => {
  ...
  const lif: LifConfig = {
    ...
    refractoryTicks: 0,
    tauPredictiveTicks: 50,
    predictiveThresholdReduction: 0.5,
  };
  ...
  countSim.step();
  assert.ok(
    countSim.predictiveView()[cb]! > 0,
    'count mode must depolarise from a single weak-weight delivery -- it ignores weight entirely',
  );
  ...
  const weightedSim = Simulation.create(lif, {
    ...
    segments: {
      segmentsPerNeuron: 1,
      coincidenceThreshold: 1,
      voteReferenceWeight: referenceWeight,
    },
  });
  ...
  weightedSim.step();
  assert.equal(
    weightedSim.predictiveView()[wb]!,
    0,
    'weighted mode must NOT depolarise from the same weak-weight delivery alone (0.05/0.8 < the threshold-1 requirement)',
  );
});
      ...
      sim.buildColumns(1n, [
        columnConfig({
          segments: {
            segmentsPerNeuron: 1,
            coincidenceThreshold: 1,
            voteReferenceWeight: 0.5,
          },
        }), // disagrees: 0.5 vs the scheduler's 0.8
      ]),
    /segments/,
  );
});
...
test('Simulation.buildColumns refuses a column whose inhibition disagrees with the scheduler-wide one (docs/findings.md finding 21)', () => {
  const sim = Simulation.create(
    { tauMTicks: 5, vRest: 0, vReset: 0, refractoryTicks: 0 },
    {
      ...
      connectionThreshold: 0.3,
      synapseCapPerNeuron: 1,
      inhibition: { neighbourhoodSize: 4, k: 1 },
    },
  );
  assert.throws(
...
test('Simulation.buildColumns refuses a column claiming k-WTA competition when the simulation runs no inhibition at all (docs/findings.md finding 21)', () => {
  ...
  assert.doesNotThrow(() =>
    sim.buildColumns(1n, [columnConfig({ neighbourhoodSize: 4, k: 4 })]),
  );
});
...
test('Simulation.membraneView is cached per epoch, not re-minted on every access (Requirement 8.4)', () => {
  ...
  assert.notEqual(
    third,
    first,
    'growth must mint a fresh view rather than returning a stale cached one',
  );
});
...
test('Simulation homeostaticScaling measurably rescales weight through the real compiled addon, with no caller-driven sweep call (Requirement 9.2, 9.6)', () => {
  ...
  function buildAndProbe(withHomeostasis: boolean): boolean {
    const options: SimulationOptions = {
      ...
      synapseCapPerNeuron: 1,
      ...(withHomeostasis
        ? { homeostaticScaling: { targetTotalWeight: 0.5, intervalTicks: 1 } }
        : {}),
    };
    const sim = Simulation.create(lif, options);
  ...
  assert.equal(
    buildAndProbe(false),
    true,
    'without homeostatic scaling, weight 0.8 alone must still cross threshold',
  );
  assert.equal(
    buildAndProbe(true),
    false,
    'with homeostatic scaling active, the rescaled-down synapse must no longer cross threshold alone -- proving the sweep ran with no caller-driven maybe_apply call',
  );
});
...
test('Simulation structuralPlasticity prunes a weak synapse through the real compiled addon, with no caller-driven sweep call (Requirement 9.2, 9.6)', () => {
  ...
  assert.equal(
    probe.includes(post),
    false,
    "the weak synapse must have been pruned automatically inside step(), so post never receives a's delivery",
  );
});
...
test('Simulation.growth allocates neurons automatically through the real compiled addon, with no caller-driven apply_growth call (Requirement 1 AC1, Requirement 2)', () => {
  ...
  assert.ok(
    sim.liveNeuronCount() > 10,
    'sustained collisions must have triggered automatic growth inside step(), with no separate apply_growth call',
  );
  assert.ok(
    sim.growthEventCount() >= 1,
    'a growth event must be observable via growthEventCount()',
  );
});
...
test('Simulation.create rejects growth configured together with threadCount > 1 (NET-10 partitioned-mode restriction)', () => {
  ...
  assert.throws(
    ...
    /growth is not supported together with threadCount/,
  );
});
...
test("without densityTarget, a partially-filled trailing group of grown neurons has no real competition (Fix 1's defect, reproduced through the real addon)", () => {
  ...
  assert.ok(
    grownCount > 0 && grownCount < 4,
    `expected a partially-filled trailing group (1-3 members), got ${grownCount}`,
  );
  ...
  assert.equal(
    trailingWinners,
    grownCount,
    `without densityTarget, ALL ${grownCount} members of the under-filled trailing group must win -- no real competition at all, the exact sparsity violation Fix 1 closes`,
  );
});
...
test('with densityTarget, a partially-filled trailing group of grown neurons respects it (Fix 1, through the real addon)', () => {
  ...
  assert.equal(
    baseWinners,
    2,
    "the full 4-member base group must still cap at densityTarget*4 = 2 winners, unchanged from today's fixed-k behaviour",
  );
  assert.ok(
    trailingWinners < grownCount,
    `the trailing group must cap below its own full membership (${grownCount}) instead of every member winning -- got ${trailingWinners}`,
  );
});
...
test('Simulation.reward measurably changes a plasticity outcome through the real compiled addon (Requirement 15.1, 15.2)', () => {
  ...
  assert.equal(
    trainThenProbe(false),
    false,
    'with no reward, STDP potentiation is gated to zero (modulator=0), so the synapse must stay sub-threshold',
  );
  assert.equal(
    trainThenProbe(true),
    true,
    'with reward driving the dopamine channel, repeated causal pairing must potentiate the synapse enough to cross threshold on a single later delivery',
  );
});
...
test('Simulation.injectModulator/modulatorLevels round-trip a value through the real compiled addon (Requirement 15.2, 15.5)', () => {
  ...
  assert.deepEqual(
    sim.modulatorLevels(),
    [0, 0, 0, 0],
    'an unstimulated simulation starts with every channel at zero',
  );
  ...
  assert.equal(
    ...
    3.5,
    'injectModulator must reach exactly the channel requested',
  );
  assert.equal(
    levels[DOPAMINE],
    0,
    'injecting one channel must not touch another',
  );
});
...
test('Simulation.injectModulator broadcasts identically across partitions (Requirement 15.3, 15.4)', () => {
  ...
  assert.deepEqual(
    buildAndRun(4, neuronCount),
    buildAndRun(),
    "a single injectModulator call must reach every partition, reproducing threadCount 1's spike sequence exactly",
  );
});
...
test('Simulation: reward/modulator field round-trips through snapshot/restore exactly (Requirement 15.6)', () => {
  ...
  try {
    ...
    assert.deepEqual(
      restored.modulatorLevels(),
      levelsBeforeSave,
      "restored simulation's modulator levels must match exactly, including partial decay",
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test('Simulation.runConsolidation advances currentTick and replays recorded activity (Requirement 12.1, 12.2)', () => {
  ...
  assert.equal(
    report.replayedSpikes,
    2,
    'both recorded spikes (a then b) must be replayed',
  );
  assert.ok(
    sim.currentTick() > tickBeforeConsolidation,
    'consolidation must advance the tick counter for every tick of replay it performs',
  );
});
...
test('Simulation.runConsolidation rejects an out-of-range pruneFloor (Requirement 12.3)', () => {
  ...
  sim.allocateNeuron(0.5, 1);
  assert.throws(() =>
    sim.runConsolidation(1n, consolidationConfig({ pruneFloor: 1.5 })),
  );
});
...
test('Simulation: consolidation round-trips through snapshot/restore (advanced tick and topology both survive)', () => {
  ...
  try {
    ...
    assert.equal(
      restored.currentTick(),
      tickAfterConsolidation,
      'the post-consolidation tick must round-trip exactly',
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
...
test('Simulation.coordsView reflects real column-placed coordinates (NET-3, Requirement 1)', () => {
  ...
  sim.buildColumns(1n, [
    columnConfig({ neuronCount: 3, baseX: 10, baseY: 20, baseZ: 30 }),
  ]);
...
test('Simulation.thresholdView, refractoryView and lastSpikeView reflect real neuron state (Requirement 1)', () => {
  ...
  assert.equal(
    sim.lastSpikeView()[a],
    0xffffffff,
    'u32::MAX sentinel before any spike',
  );
  ...
  assert.equal(
    sim.lastSpikeView()[a],
    0,
    'lastSpikeView must reflect the tick a actually spiked at',
  );
  assert.ok(
    sim.refractoryView()[a]! > sim.currentTick(),
    'a must be refractory immediately after spiking',
  );
});
...
test('Simulation.adaptationView defaults to zero with no adaptation configured (NEU-8, Requirement 1.4 backward compatibility)', () => {
  ...
  assert.equal(
    sim.adaptationView()[a],
    0,
    'adaptation must stay exactly zero when tauAdaptationTicks/adaptationIncrement are never configured',
  );
});
...
test("Simulation synapse bulk views expose a connected synapse's real data, and occupied filters unallocated slots (SYN-1/2/3, Requirement 2)", () => {
  ...
  assert.equal(
    Math.floor(synId / capPerNeuron),
    a,
    "a synapse id's source must resolve via id / capPerNeuron",
  );
  ...
  assert.equal(
    occupied[synId],
    1,
    'the slot actually connected must be marked occupied',
  );
  ...
  assert.equal(
    occupied[unusedSlot],
    0,
    'an unallocated slot within the same block must be marked unoccupied',
  );
});
...
test('Simulation.rasterBytes exports real recorded spikes (OBS-3, Requirement 3)', () => {
  ...
  assert.ok(
    bytes.length > 14,
    'a raster with at least one recorded spike must be longer than the bare header',
  );
  assert.deepEqual(
...
test("Simulation.rasterBytes exports real recorded spikes in partitioned mode too, reproducing threadCount 1's raster exactly (Phase 7 Requirement 1(d))", () => {
  ...
  assert.ok(
    single.length > 14,
    'a raster with recorded spikes must be longer than the bare header',
  );
  assert.deepEqual(
    Array.from(partitioned),
    Array.from(single),
    "threadCount 2's exported raster must reproduce threadCount 1's exactly, byte for byte",
  );
});
...
test('Simulation attachProbe/readProbe/detachProbe round-trip real spike and membrane data through the addon (OBS-1, Requirement 4)', () => {
  ...
  assert.equal(
    sim.readProbe(a),
    undefined,
    'a detached probe must no longer be readable',
  );
});
...
test('Simulation attachProbe with recordSegments records real per-tick segment activity (Requirement 6)', () => {
  ...
  assert.ok(
    data.segmentSamples && data.segmentSamples.length > 0,
    'segment activity must have been recorded',
  );
  const sample = data.segmentSamples![0]!;
  assert.equal(sample.segment, 0);
  assert.equal(sample.active, 2);
  assert.ok(
    sample.depolarisation > 0,
    '2 of 2 must reach the configured threshold',
  );
});
...
test('Simulation firingRate/predictionAccuracy/metricsSnapshot report real values through the addon (OBS-2, Requirement 5)', () => {
  ...
  assert.ok(
    sim.firingRate() > 0,
    'a fired repeatedly under sustained stimulation',
  );
...
test('Simulation.attachProbe/readProbe work in partitioned mode, routed to the partition that owns the neuron (Phase 7 Requirement 1(d))', () => {
  ...
  const handles = sim.buildColumns(1n, [
    columnConfig({ neuronCount: columnSize, baseY: 0 }),
    columnConfig({ neuronCount: columnSize, baseY: 100 }),
  ]);
  ...
  assert.notEqual(
    data,
    undefined,
    'a probe attached to a neuron in a non-zero partition must still be readable',
  );
  assert.deepEqual(data!.spikeTimes, [0]);
  assert.equal(data!.membraneTrace!.length, 1);
  ...
  assert.equal(
    sim.readProbe(target),
    undefined,
    'a detached probe must no longer be readable in partitioned mode either',
  );
});
...
test('Simulation.firingRate sums raw spike counts across partitions rather than averaging their rates (Phase 7 Requirement 1(d))', () => {
  ...
  const handles = sim.buildColumns(1n, [
    columnConfig({ neuronCount: 1, baseY: 0 }),
    columnConfig({ neuronCount: 9, baseY: 100 }),
  ]);
  ...
  assert.ok(
    ...
    `expected the correctly-summed combined rate 0.1, got ${rate} (0.5 would indicate averaging per-partition rates instead)`,
  );
});
...
test("Simulation.predictionAccuracy reproduces threadCount 1's value exactly under partitioning, with real dendritic prediction engaged (Phase 7 Requirement 1(d))", () => {
  ...
  assert.equal(
    partitioned,
    single,
    "threadCount 2's combined predictionAccuracy must exactly reproduce threadCount 1's, not merely be close to it",
  );
});
...
test('TypeScript strict mode is enabled and the FFI surface names no `any` (Requirement 1.5)', () => {
  ...
  assert.equal(
    ...
    true,
    'strict must be enabled for the whole workspace',
  );
  ...
  for (const relative of boundaryFiles) {
    ...
    assert.doesNotMatch(
      ...
      `${relative} must not name \`any\` at the FFI boundary`,
    );
  }
});
...
function stdpModulationOptions(
  stdpModulation?: NonNullable<
    SimulationOptions['plasticity']
  >['stdpModulation'],
): SimulationOptions {
  return {
    maxDelay: 2,
...
test('PlasticityConfig.stdpModulation: a mapped level reaches the kernel through the real addon, and unset changes nothing (LRN-2, PLAN.md C5)', () => {
  ...
  assert.equal(
    atReference,
    weightAfterTraining(unset, 1.0),
    "with the channel exactly at the map's reference the scale is 1.0, so the run must be bit-identical to the hook unset",
  );

  const doubled = weightAfterTraining(mapped, 2.0);
  assert.ok(
    doubled > atReference,
    `a level of 2.0 doubles a_plus, so causal pairing must potentiate more: ${doubled} vs ${atReference}`,
  );

  // VAL-9's ablation shape: with the hook unset the same level change reaches nothing, because nothing reads the channel.
  assert.equal(
    weightAfterTraining(unset, 2.0),
    weightAfterTraining(unset, 0.5),
    'hook unset: noradrenaline is read by nothing, so its level cannot matter',
  );
});
...
function c9Run(options: SimulationOptions, acetylcholine: number) {
  ...
  const slots = occupied.reduce<number[]>(
    (acc, o, i) => (o === 1 ? [...acc, i] : acc),
    [],
  );
...
test('SimulationOptions.transmissionModulation: a recurrent gate crosses the addon, and unset changes nothing (PLAN.md C9)', () => {
  ...
  assert.equal(
    unset.stats,
    null,
    'no gate configured reads back as null, not as a gate that did nothing',
  );

  const atReference = c9Run(c9Options({ recurrent: suppressRecurrent }), 1.0);
  assert.deepEqual(
    atReference.weights,
    c9Run(c9Options(), 1.0).weights,
    "at the map's reference the scale is exactly 1.0, so the run is bit-identical to the gate unset",
  );
  assert.ok(
    atReference.stats !== null,
    'a configured gate reads back its counters',
  );
  assert.equal(
    atReference.stats.scaled,
    0,
    'and at the reference it scaled nothing',
  );

  const gated = c9Run(c9Options({ recurrent: suppressRecurrent }), 3.0);
  assert.ok(gated.stats !== null);
  assert.ok(
    gated.stats.events > 0,
    'the recurrent deliveries must actually have reached the gate',
  );
  assert.equal(
    gated.stats.events,
    gated.stats.silenced,
    'at level 3.0 this map clamps to 0: every gated delivery is silenced outright',
  );
  assert.equal(
    gated.stats.minLevel[ACETYLCHOLINE],
    3.0,
    "and the level a delivery read is reported, which is what a map's reference must be measured against",
  );
});
...
test('SimulationOptions.transmissionModulation: an unusable map is refused at construction with a clean error, not a panic (ENG-9)', () => {
  ...
  const refused = (
    map: NonNullable<SimulationOptions['transmissionModulation']>,
    because: RegExp,
  ) => assert.throws(() => Simulation.create(lif, c9Options(map)), because);

  refused(
    { recurrent: { ...suppressRecurrent, channel: 4 } },
    /transmissionModulation: .*channel/,
  );
  refused(
    { recurrent: { ...suppressRecurrent, gain: Number.NaN } },
    /transmissionModulation: .*NaN or infinite/,
  );
  refused(
    { recurrent: { ...suppressRecurrent, min: 5.0, max: 1.0 } },
    /transmissionModulation: .*min exceeds its max/,
  );
  // NEU-4 / README invariant 3: a negative scale would turn an excitatory contact inhibitory.
  refused(
    { recurrent: { ...suppressRecurrent, min: -0.5 } },
    /transmissionModulation: .*min is negative/,
  );
  refused(
    { feedforward: { ...suppressRecurrent, min: -0.5 } },
    /transmissionModulation: .*min is negative/,
  );
  ...
  assert.doesNotThrow(() =>
    Simulation.create(
      lif,
      c9Options({ recurrent: { ...suppressRecurrent, min: 0.0 } }),
    ),
  );
});
...
test('PlasticityConfig.recurrent: a second rule instance for the dendritic pathway crosses the addon (PLAN.md C8/C9)', () => {
  ...
  assert.equal(
    routed.weights[0],
    unset.weights[0],
    'the feedforward synapse keeps running the default chain, untouched',
  );
  assert.ok(
    routed.weights[1]! > unset.weights[1]!,
    `high acetylcholine must enhance LTP on the recurrent chain only: ${routed.weights[1]} vs ${unset.weights[1]}`,
  );
  ...
  assert.deepEqual(
    atReference.weights,
    c9Run(c9Options(), 1.0).weights,
    "at the map's reference the two chains are the same curve, bit for bit",
  );
});
...
test('PlasticityConfig.recurrent: its own fields are validated, with the offending path named (ENG-9)', () => {
  ...
  assert.throws(
    () =>
      Simulation.create(
        lif,
        c9Options(undefined, { ...ruleOnly, modulatorChannel: 9 }),
      ),
    /recurrent\.modulatorChannel/,
  );
  assert.throws(
    () =>
      Simulation.create(
        lif,
        c9Options(undefined, { ...ruleOnly, gainModulatorChannel: 9 }),
      ),
    /recurrent\.gainModulatorChannel/,
  );
});
...
test('PlasticityConfig.stdpModulation: an unusable map is refused at construction with a clean error, not a panic (ENG-9)', () => {
  ...
  const refused = (
    ...
    assert.throws(
      () => Simulation.create(lif, stdpModulationOptions(stdpModulation)),
      because,
    );

  refused(
    { aPlus: { ...amplitudeMap, channel: 4 } },
    /stdpModulation: .*channel/,
  );
  refused(
    { aPlus: { ...amplitudeMap, gain: Number.NaN } },
    /stdpModulation: .*NaN or infinite/,
  );
  refused(
    { aPlus: { ...amplitudeMap, min: 5.0, max: 1.0 } },
    /stdpModulation: .*min exceeds its max/,
  );
  ...
  refused(
    { tauPlus: { ...amplitudeMap, min: 0.0 } },
    /stdpModulation: .*min <= 0/,
  );
  refused(
    { windowTicks: { ...amplitudeMap, min: -1.0 } },
    /stdpModulation: .*min <= 0/,
  );
  assert.doesNotThrow(() =>
    Simulation.create(
      lif,
      stdpModulationOptions({ aMinus: { ...amplitudeMap, min: -1.0 } }),
    ),
  );
});
