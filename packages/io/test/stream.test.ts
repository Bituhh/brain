...
test("streamThrough advances the simulation's tick counter by ticksPerInput per yielded step", () => {
  ...
  assert.equal(
    sim.currentTick(),
    before + 9,
    '3 inputs * 3 ticksPerInput must advance the tick counter by exactly 9',
  );
});
...
test('streamThrough: plasticity remains active throughout, with no train/inference mode switch (Requirement 9.2, IO-4)', () => {
  ...
  assert.equal(
    ...
    false,
    'with no reward, STDP potentiation is gated to zero, so the internal synapse must stay sub-threshold',
  );
  assert.equal(
    trainThenProbe(true),
    true,
    'with reward active throughout streaming, repeated causal pairing must potentiate the internal synapse enough to cross threshold on a single later delivery',
  );
});
...
test('streamThrough: stopping mid-stream and snapshotting round-trips correctly (Requirement 9.4)', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      sim.currentTick(),
      'a simulation snapshotted mid-stream must restore at the exact same tick',
    );
  } finally {
