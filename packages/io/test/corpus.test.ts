...
test("corpus fixture uses only characters in the text encoder's SUPPORTED_ALPHABET", () => {
  ...
  for (let i = 0; i < corpus.length; i++) {
    const char = corpus[i]!;
    assert.ok(alphabet.has(char), `character ${JSON.stringify(char)} at offset ${i} is outside SUPPORTED_ALPHABET`);
  }
});
...
test("trigram baseline achieves plausible, non-degenerate accuracy on the corpus (informational floor for Requirement 13.3/13.4)", () => {
  ...
  assert.ok(acc.accuracy > 0.3, `trigram baseline accuracy too low to be plausible: ${acc.accuracy}`);
});
