...
function fourActionColumnConfig(): ColumnConfig {
  return {
    ...
    neighbourhoodSize: 4,
    k: 4,
    // No internal wiring and no dendritic segments in this closed-loop
    // ablation -- must match `SimulationOptions`, which also omits
...
test("closed loop: the network's decoded action reliably reaches the distinguishing cell (Requirement 16.3)", () => {
  assert.equal(
    runTask(true, '!'),
    true,
    "with actions applied, always decoding 'right' must reach the distinguishing cell 4 columns away within 5 steps",
  );
});
...
test("disconnected loop: actions sampled independently of the network's output do not reach the distinguishing cell (Requirement 16.5's ablation)", () => {
  assert.equal(
    runTask(false, '!'),
    false,
    "with the decoded action never applied, the fixed non-'right' action sequence must not happen to reach the distinguishing cell either",
  );
});
...
test('the closed loop measurably outperforms the disconnected one on the disambiguation task (Requirement 16.5)', () => {
  ...
  assert.ok(
    closed && !disconnected,
    `closing the loop must be what distinguishes success from failure on this task: closed=${closed}, disconnected=${disconnected}`,
  );
});
...
test('the task is genuinely about grounding, not encoder trivia: two grids differing only at the distinguishing cell are told apart only by an agent that visits it', () => {
  ...
  assert.ok(
    sawGridA && sawGridB,
    "an agent that closes the loop must be able to observe either grid's distinguishing symbol, whichever grid it is placed in",
  );
});
