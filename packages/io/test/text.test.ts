...
import { encodeChar, encodeWord, tokenizeWords, SUPPORTED_ALPHABET, type CharEncoderConfig, type WordEncoderConfig } from "../src/encoders/text.ts";
import { overlap } from "../src/sdr.ts";

function charConfig(overrides: Partial<CharEncoderConfig> = {}): CharEncoderConfig {
  return { width: 400, density: 0.05, ...overrides };
}

function wordConfig(overrides: Partial<WordEncoderConfig> = {}): WordEncoderConfig {
...
test("encodeChar: two distinct characters have low overlap relative to density (Requirement 5.4)", () => {
  ...
  assert.ok(overlap(e, z) < expectedActive * 0.5, `'e' and 'z' must not overlap heavily, got ${overlap(e, z)}`);
});
...
test("encodeChar with context: the same character in different contexts produces different SDRs (Requirement 5.1)", () => {
  ...
  assert.notDeepEqual(eInThe.activeBits, eInHe.activeBits, "'e' in \"the\" vs. 'e' in \"he\" must be distinguishable once context is folded in");
});
...
test("tokenizeWords splits on whitespace and punctuation using plain string logic (Requirement 5.2)", () => {
  assert.deepEqual(tokenizeWords("the quick, brown fox!"), ["the", "quick", "brown", "fox"]);
  assert.deepEqual(tokenizeWords("  multiple   spaces  "), ["multiple", "spaces"]);
