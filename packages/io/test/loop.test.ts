...
function fourActionColumnConfig(): ColumnConfig {
  return {
    ...
    }, // no internal wiring -- this test drives neurons directly
    neighbourhoodSize: 4,
    k: 4,
...
test('runSensorimotorLoop closes the loop: a decoded action changes the next observation (Requirement 16.3 smoke test, IO-5)', () => {
  ...
  assert.ok(
    reachedDistinguishingCell,
    "repeatedly decoding 'right' must move the cursor and eventually reach the distinguishing cell -- proving the loop actually closes, not just decodes",
  );
});
...
test("runSensorimotorLoop: an action that is never taken leaves the environment unchanged (Requirement 16.3's converse)", () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    assert.equal(
      step.action,
      undefined,
      'with nothing stimulated, no candidate should clear a 0.9 confidence threshold',
    );
  }
  assert.deepEqual(
    world.cursor,
    before,
    'the cursor must not move when the network never commits to an action',
  );
});
...
test('runSensorimotorLoop yields the observation it acted on, not the one after', () => {
  ...
  assert.deepEqual(
    step.observation,
    firstObservation,
    "the yielded observation must be what the network actually saw, before this step's action was taken",
  );
});
