...
function config(
  overrides: Partial<ScalarEncoderConfig> = {},
): ScalarEncoderConfig {
  return {
    min: 0,
    max: 100,
...
test('encodeScalar: close values overlap substantially, far values overlap little or not at all (Requirement 3.2, IO-1)', () => {
  ...
  assert.ok(
    overlap(base, close) > cfg.activeBits * 0.5,
    `close values (50 vs 51) must overlap substantially, got ${overlap(base, close)}`,
  );
  assert.ok(
    overlap(base, far) < cfg.activeBits * 0.2,
    `far values (50 vs 99) must overlap little, got ${overlap(base, far)}`,
  );
});
...
test('encodeScalar: overlap decreases monotonically as values move apart', () => {
  ...
  for (const delta of [5, 10, 20, 30, 40]) {
    ...
    assert.ok(
      o <= previousOverlap,
      `overlap must not increase as distance grows: at delta=${delta}, overlap=${o}, previous=${previousOverlap}`,
    );
    previousOverlap = o;
  }
});
...
test('encodeScalar rejects an invalid config', () => {
  assert.throws(
    () => encodeScalar(config({ max: 0, min: 100 }), 50),
    RangeError,
  );
  assert.throws(() => encodeScalar(config({ activeBits: 0 }), 50), RangeError);
  assert.throws(
    () => encodeScalar(config({ activeBits: 300 }), 50),
    RangeError,
  );
});
