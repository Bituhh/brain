...
test('runCharPredictionTrial runs end to end on a small corpus slice and returns a well-formed result', () => {
  const result = runCharPredictionTrial(corpus, 1n, {
    ...DEFAULT_CONFIG,
    slidingWindow: 100,
  });
  assert.equal(result.seed, 1n);
  assert.ok(
    Number.isFinite(result.networkAccuracy) &&
      result.networkAccuracy >= 0 &&
      result.networkAccuracy <= 1,
  );
  assert.ok(
    Number.isFinite(result.trigramAccuracy) &&
      result.trigramAccuracy >= 0 &&
      result.trigramAccuracy <= 1,
  );
...
test("rewardSignal omitted leaves the network deterministic (RUN-3) across repeated runs, and 'correctness' measurably changes it", () => {
  ...
  assert.deepEqual(
    baselineB,
    baselineA,
    'the unconfigured (rewardSignal omitted) path must be bit-identical across repeated runs of the same seed/config',
  );

  const rewarded = runCharPredictionTrial(corpus, 1n, {
    ...config,
    rewardSignal: 'correctness',
  });
  assert.ok(
    Number.isFinite(rewarded.networkAccuracy) &&
      rewarded.networkAccuracy >= 0 &&
      rewarded.networkAccuracy <= 1,
  );
  assert.notDeepEqual(
    rewarded,
    baselineA,
    "configuring rewardSignal: 'correctness' must produce a measurably different result from the unconfigured baseline",
  );
});
...
test('inhibitionHomeostasis omitted leaves the network deterministic (RUN-3) across repeated runs, and a configured target measurably changes it', () => {
  ...
  assert.deepEqual(
    baselineB,
    baselineA,
    'the unconfigured (inhibitionHomeostasis omitted) path must be bit-identical across repeated runs of the same seed/config',
  );
  ...
  assert.ok(
    Number.isFinite(tuned.networkAccuracy) &&
      tuned.networkAccuracy >= 0 &&
      tuned.networkAccuracy <= 1,
  );
  assert.notDeepEqual(
    tuned,
    baselineA,
    "configuring inhibitionHomeostasis with a target far from today's fixed k/size ratio must produce a measurably different result from the unconfigured baseline",
  );
});
...
test('growth/structuralPlasticity omitted leaves the network deterministic (RUN-3) across repeated runs, and configuring them measurably changes it', () => {
  ...
  assert.deepEqual(
    baselineB,
    baselineA,
    'the unconfigured (growth/structuralPlasticity omitted) path must be bit-identical across repeated runs of the same seed/config',
  );
  ...
  assert.ok(
    Number.isFinite(grown.networkAccuracy) &&
      grown.networkAccuracy >= 0 &&
      grown.networkAccuracy <= 1,
  );
  assert.notDeepEqual(
    grown,
    baselineA,
    'configuring growth+structuralPlasticity must produce a measurably different result from the unconfigured baseline',
  );
});
...
test('silentSynapses omitted leaves structural plasticity deterministic (RUN-3), and configuring it measurably changes the result', () => {
  ...
  assert.deepEqual(
    ungatedB,
    ungatedA,
    'the ungated path must be bit-identical across repeated runs of the same seed/config',
  );

  const gated = runCharPredictionTrial(corpus, 1n, {
    ...config,
    silentSynapses: { unsilenceWeight: 0.15 },
  });
  assert.notDeepEqual(
    gated,
    ungatedA,
    'configuring silentSynapses must produce a measurably different result from the ungated baseline',
  );
});
...
test('structuralStats reports sprouts, and unsilencing only happens when something can unsilence', () => {
  ...
  assert.ok(
    ungated.unsilencedTotal > 0,
    'without a gate, delivering sprouts must be unsilenced',
  );
  assert.ok(ungated.occupiedNow > 0);

  // Gate on, weights frozen (no STDP): nothing can ever unsilence.
  const frozen = runCharPredictionTrial(corpus, 1n, {
    ...base,
    silentSynapses: { unsilenceWeight: 0.2 },
  }).structuralStats!;
  assert.ok(frozen.sproutedTotal > 0);
  assert.equal(
    frozen.unsilencedTotal,
    0,
    'with weights frozen below the unsilence weight, nothing may unsilence',
  );
  ...
  assert.ok(
    frozen.silentNow > 0 && frozen.silentNow <= frozen.sproutedTotal,
    'surviving sprouts must still be silent, and no more than were sprouted',
  );
  ...
  assert.ok(
    learning.unsilencedTotal > 0,
    'with STDP on and a low unsilence weight, some sprout must unsilence',
  );
});
...
test('runCharPredictionTrial reports progress in increasing steps, and structuralStats is absent without structural plasticity', () => {
  ...
  const result = runCharPredictionTrial(
    corpus,
    1n,
    { ...DEFAULT_CONFIG, slidingWindow: 100 },
    (done, of) => {
      seen.push(done);
      total = of;
    },
  ...
  assert.ok(
    seen.length > 0,
    'progress must be reported at least once on a 400-character slice',
  );
  assert.deepEqual(
    seen,
    [...seen].sort((a, b) => a - b),
...
test("B5's config options (voteReferenceWeight, predictiveLearningTarget, homeostaticScaling, coincidenceThreshold) each reach the scheduler, and scaling only matters once votes are weighted", () => {
  ...
  assert.deepEqual(
    baselineB,
    baselineA,
    'the unconfigured path must be bit-identical across repeated runs of the same seed/config',
  );
  ...
  for (const [name, variant] of Object.entries(variants)) {
    ...
    assert.notDeepEqual(
      result,
      baselineA,
      `configuring ${name} must produce a measurably different result from the unconfigured baseline`,
    );
  }
  ...
  const scaling = {
    homeostaticScaling: { targetTotalWeight: 2.0, intervalTicks: 20 },
  };
  assert.deepEqual(
    runCharPredictionTrial(corpus, 1n, { ...config, ...scaling }),
    baselineA,
    'in count mode, homeostatic scaling must have no effect',
  );
...
test('a consolidation cadence sleeps on schedule, replays real events, prunes real synapses, and is inert when it never fires (PLAN.md C1)', () => {
  ...
  assert.equal(
    baseline.consolidationStats,
    undefined,
    'an unconfigured cadence must report no consolidation stats at all',
  );
  ...
  const slept = runCharPredictionTrial(corpus, 1n, {
    ...config,
  ...
  assert.ok(
    stats !== undefined,
    'a configured cadence must report consolidation stats',
  );
  assert.equal(
    stats.passes,
    3,
    '400 characters at a 100-character cadence is three sleeps: the one on the last character is skipped',
  );
  assert.ok(
    stats.replayedSpikes > 0,
    'each sleep must actually replay recorded events, not run an empty pass',
  );
  ...
  assert.deepEqual(
    runCharPredictionTrial(corpus, 1n, { ...config, consolidation: cadence }),
    slept,
    'a configured cadence must stay bit-identical across repeated runs of the same seed/config',
  );
  ...
  const pruning = runCharPredictionTrial(corpus, 1n, {
    ...config,
    consolidation: { ...cadence, pruneFloor: 0.45 },
  });
  assert.ok(
    pruning.consolidationStats!.pruned > 0,
    "a prune floor above every synapse's initial permanence must actually prune",
  );

  // A cadence wider than the corpus never fires, and a cadence that never
  // fires must leave the run exactly as it was.
  const never = runCharPredictionTrial(corpus, 1n, {
    ...config,
    consolidation: { ...cadence, everyCharacters: 10_000 },
  });
