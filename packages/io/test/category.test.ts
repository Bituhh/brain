...
import { encodeCategory, UnknownCategoryError, type CategoryEncoderConfig } from "../src/encoders/category.ts";
import { overlap } from "../src/sdr.ts";

const CATEGORIES = ["red", "green", "blue", "yellow", "purple", "orange", "cyan", "magenta"] as const;

function config(overrides: Partial<CategoryEncoderConfig<(typeof CATEGORIES)[number]>> = {}): CategoryEncoderConfig<(typeof CATEGORIES)[number]> {
...
test("encodeCategory: unrelated categories have low overlap by default (Requirement 3.4)", () => {
  ...
  for (let i = 0; i < CATEGORIES.length; i++) {
    for (let j = i + 1; j < CATEGORIES.length; j++) {
      ...
      assert.ok(overlap(a, b) < activeBitsPerCategory * 0.5, `"${CATEGORIES[i]}" and "${CATEGORIES[j]}" must not overlap heavily, got ${overlap(a, b)}`);
    }
  }
});

test("encodeCategory rejects a label outside the configured set (Requirement 3.5)", () => {
  assert.throws(() => encodeCategory(config(), "not-a-color" as never), UnknownCategoryError);
});
...
test("encodeCategory: every category is distinct from every other, pairwise, across the whole set", () => {
  ...
  for (let i = 0; i < encoded.length; i++) {
    for (let j = i + 1; j < encoded.length; j++) {
      assert.notDeepEqual(encoded[i]!.activeBits, encoded[j]!.activeBits, `${CATEGORIES[i]} and ${CATEGORIES[j]} must not collide exactly`);
    }
  }
});
