...
test("encodeLocation width is the sum of every module's x + y widths", () => {
  const config: LocationEncoderConfig = {
    modules: [
      { period: 20, width: 40, activeBits: 8 },
      { period: 5, width: 10, activeBits: 3 },
    ],
  };
...
test('nearby positions overlap substantially', () => {
  ...
  assert.ok(
    overlap(a, b) > 5,
    `one unit apart must overlap substantially, got ${overlap(a, b)}`,
  );
});
...
test("far-apart positions (within one module's period) overlap little", () => {
  ...
  assert.ok(
    overlap(a, b) < 4,
    `far apart must overlap little, got ${overlap(a, b)}`,
  );
});
...
test('a module wraps at its own period: two positions exactly one period apart produce identical sub-SDRs for that module (grid-cell-like periodicity, Requirement 6 AC3)', () => {
  ...
  assert.deepEqual(
    here.activeBits,
    onePeriodAway.activeBits,
    'positions differing by exactly one module period must produce the bit-identical encoding for that module',
  );
});
...
test("two different physical positions can share one module's phase without being the same position", () => {
  ...
  assert.deepEqual(
    a.activeBits,
    b.activeBits,
    'aliasing within one module is expected',
  );
});
...
test('multiple modules at different periods disambiguate positions that alias in only one module', () => {
  ...
  assert.notDeepEqual(
    a.activeBits,
    b.activeBits,
    "a second module at a different period must break the first module's aliasing",
  );
});
...
test('GridModule rejects a non-positive period', () => {
  assert.throws(
    () =>
      encodeLocation(
        { modules: [{ period: 0, width: 10, activeBits: 2 }] },
        { x: 0, y: 0 },
      ),
    RangeError,
  );
...
test('GridModule rejects activeBits outside (0, width]', () => {
  assert.throws(
    () =>
      encodeLocation(
        { modules: [{ period: 10, width: 10, activeBits: 0 }] },
        { x: 0, y: 0 },
      ),
    RangeError,
  );
  assert.throws(
    () =>
      encodeLocation(
        { modules: [{ period: 10, width: 10, activeBits: 11 }] },
        { x: 0, y: 0 },
      ),
    RangeError,
  );
...
test("PathIntegrator is independent of any environment's own cursor state -- it only ever sees the actions it is given", () => {
  ...
  assert.deepEqual(
    integrator.position,
    { x: -2, y: 0 },
    'path integration accumulates raw displacement, with no clamping of its own',
  );
});
