...
function locationConfig(): LocationEncoderConfig {
  return {
    modules: [{ period: LOCATION_WIDTH, width: LOCATION_WIDTH, activeBits: 1 }],
  };
}
...
function buildNetwork(sensoryBaseX: number, wireVoting: boolean) {
  ...
  const lif: LifConfig = {
    ...
    vReset: 0,
    refractoryTicks: 0,
    tauPredictiveTicks: 1000,
    predictiveThresholdReduction: THRESHOLD_REDUCTION,
  };
...
test('a sensory neuron depolarised by a specific location spikes under otherwise-insufficient drive when that location is active (Requirement 6 AC2)', () => {
  ...
  assert.equal(
    runTrial(sensoryBaseX, true, 5),
    true,
    'location x=5 (coordinate-adjacent to the sensory neuron) must depolarise it enough for the weak symbol drive to cross threshold',
  );
});
...
test("the same weak drive does NOT spike under a different, non-overlapping location (Requirement 6 AC2's disambiguation)", () => {
  ...
  assert.equal(
    ...
    false,
    'location x=15 (coordinate-distant from the sensory neuron, under a short length_scale) must not depolarise it -- the same symbol at a different location produces a different response',
  );
});
...
test('ablation: with lateral voting never wired at all, neither location depolarises the sensory neuron (Requirement 6 AC4)', () => {
  ...
  assert.equal(
    ...
    false,
    'without any voting connection, even the previously-depolarising location must fail to raise the weak drive above threshold',
  );
});
