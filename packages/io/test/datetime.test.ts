...
import { encodeDatetime, timeOfDayComponent, dayOfWeekComponent, type DatetimeEncoderConfig } from "../src/encoders/datetime.ts";
...
test("encodeDatetime width is the sum of every component's width", () => {
  const config: DatetimeEncoderConfig = { components: [timeOfDayComponent(200, 20), dayOfWeekComponent(70, 10)] };
...
test("close timestamps (same day, minutes apart) overlap substantially (Requirement 4.2)", () => {
  ...
  assert.ok(overlap(a, b) > 15, `one minute apart must overlap substantially, got ${overlap(a, b)}`);
});
...
test("far-apart timestamps within the same day overlap little (Requirement 4.2's converse)", () => {
  ...
  assert.ok(overlap(morning, evening) < 5, `20 hours apart must overlap little, got ${overlap(morning, evening)}`);
});
...
test("the same time-of-day on different days overlaps substantially (Requirement 4.3 -- the property a raw scalar encoder cannot provide)", () => {
  ...
  assert.equal(overlap(day1, day2), 20, "identical time-of-day must produce identical time-of-day encoding regardless of the date");
});
...
test("time-of-day wraps at the day boundary: 23:59 and 00:01 overlap (Requirement 4.3's wraparound mechanism)", () => {
  ...
  assert.ok(overlap(lateNight, earlyMorning) > 5, `2 minutes apart across midnight must still overlap, got ${overlap(lateNight, earlyMorning)}`);
});

test("dayOfWeek wraps at the week boundary: Saturday and Sunday overlap", () => {
  const config: DatetimeEncoderConfig = { components: [dayOfWeekComponent(70, 20)] };
  ...
  assert.ok(overlap(saturday, sunday) > 0, "adjacent days across the week boundary (Sat/Sun) must overlap");
});
...
test("multiple components combine independently: differing only in one component still shares the other's bits", () => {
  ...
  assert.ok(overlap(mondayMorning, tuesdayMorning) >= 20, `shared time-of-day component must overlap fully even on different days, got ${overlap(mondayMorning, tuesdayMorning)}`);
});
