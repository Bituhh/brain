...
function config(overrides: Partial<GridWorldConfig> = {}): GridWorldConfig {
  return { width: 5, height: 5, seed: 42, symbols: ["x", "y", "z"], ...overrides };
}
...
test("GridWorld: different seeds can produce different layouts", () => {
  ...
  assert.notDeepEqual(cellsA, cellsB, "two different seeds producing an identical cell trace across 5 moves is not expected");
});
...
test("GridWorld.act changes the observed cell (Requirement 16.3 -- a closed loop, not a decorative action)", () => {
  const world = new GridWorld(config({ distinguishingCell: { x: 4, y: 2, symbol: "!" }, startX: 0, startY: 2 }));
  const before = world.observe().cell;
  for (let i = 0; i < 4; i++) {
    world.act("right");
  }
  const after = world.observe().cell;
  assert.equal(after, "!", "moving the cursor onto the distinguishing cell must be reflected in the next observation");
...
test("GridWorld clamps the cursor at the grid's edges rather than wrapping or erroring", () => {
  ...
  world.act("left");
  world.act("up");
  assert.deepEqual(world.cursor, { x: 0, y: 0 }, "moving off the top-left edge must clamp, not wrap or throw");
});
...
test("GridWorld rejects a distinguishingCell outside the grid", () => {
  assert.throws(() => new GridWorld(config({ distinguishingCell: { x: 100, y: 100, symbol: "!" } })), RangeError);
});
...
test("GridWorld: the distinguishing cell is reachable only by actually moving the cursor there", () => {
  ...
  const mover = new GridWorld(config({ distinguishingCell, startX: 0, startY: 0 }));
  const stationary = new GridWorld(config({ distinguishingCell, startX: 0, startY: 0 }));
  ...
  assert.ok(moverSawIt, "an agent that actually moves toward the distinguishing cell must eventually observe it");
  assert.ok(!stationarySawIt, "an agent that never moves toward it must never observe it");
});
