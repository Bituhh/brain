...
test('probeData round-trips with no optional streams enabled', () => {
  const message: ServerMessage = {
    type: 'probeData',
    neuron: 3,
    spikeTimes: new Uint32Array([1, 2]),
    membraneTrace: undefined,
    segmentSamples: undefined,
  };
  const decoded = roundTrip(message) as typeof message;
...
test('error round-trips a UTF-8 message exactly', () => {
  const message: ServerMessage = {
    type: 'error',
    message: 'neuron index 42 is out of range (☃ unicode too)',
  };
  assert.deepEqual(roundTrip(message), message);
});
...
test('setStateStride and setMetricsCadence round-trip exactly', () => {
  assert.deepEqual(roundTrip({ type: 'setStateStride', stride: 10 }), {
    type: 'setStateStride',
    stride: 10,
  });
  assert.deepEqual(
    roundTrip({ type: 'setMetricsCadence', intervalTicks: 50 }),
    { type: 'setMetricsCadence', intervalTicks: 50 },
  );
});
