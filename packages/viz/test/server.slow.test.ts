...
function connectTestClient(port: number): Promise<TestClient> {
  return new Promise((resolve, reject) => {
    ...
    const client: TestClient = {
      send(msg) {
        socket.write(
          Buffer.from(encodeFrame(OPCODE.binary, encode(msg), true)),
        );
      },
      nextMessage() {
        ...
        for (;;) {
          ...
          const msg = await Promise.race([
            ...
            new Promise<never>((_, rej) =>
              setTimeout(() => rej(new Error('waitFor timed out')), remaining),
            ),
          ]);
...
async function withServer(
  sim: Simulation,
  run: (server: VizServer) => Promise<void>,
): Promise<void> {
...
test('a connecting client receives topology for the real network (Requirement 7.2)', async () => {
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      const neurons = (await client.waitFor(
...
test('the server ticks automatically once started (Requirement 7.4, not paused by default)', async () => {
  ...
  await withServer(sim, async (server) => {
    const client = await connectTestClient(server.port);
    try {
      const tick = (await client.waitFor((m) => m.type === 'tick')) as Extract<
        ServerMessage,
        { type: 'tick' }
      >;
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      ...
      assert.equal(
        tick.tick,
        beforeTick + 1,
        'stepOnce must advance exactly one tick while paused',
      );
      ...
      assert.equal(
        sawAnotherTick,
        false,
        'no tick should be broadcast on its own while paused',
      );
    } finally {
      client.close();
    }
...
test('stimulate reaches the real simulation through the control channel (Requirement 8.1)', async () => {
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      ...
      >;
      assert.deepEqual(
        Array.from(tick.spiked),
        [a],
        'the stimulated neuron must spike on the very next step',
      );
    } finally {
      client.close();
    }
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      ...
      const reply = await client.waitFor(
        (m) => m.type === 'metricsSnapshot' || m.type === 'error',
      );
      assert.equal(
        reply.type,
        'metricsSnapshot',
        'reward/injectModulator must not produce an error reply',
      );
    } finally {
      client.close();
    }
...
test('a non-primary connection may still attach a probe (Requirement 8.3, observation-only actions are harmless)', async () => {
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      ...
      const probeData = (await observer.waitFor(
        (m) => m.type === 'probeData',
        3000,
      )) as Extract<ServerMessage, { type: 'probeData' }>;
...
test('requestRaster returns the real exported raster bytes (Requirement 7, OBS-3)', async () => {
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      ...
      const raster = (await client.waitFor(
        (m) => m.type === 'rasterExport',
      )) as Extract<ServerMessage, { type: 'rasterExport' }>;
...
test('starting a server against a partitioned simulation now works end-to-end -- topology, ticking, probes and raster all served for real (Phase 7 Requirement 1(e))', async () => {
  ...
  await withServer(sim, async (server) => {
    ...
    try {
      ...
      assert.equal(
        neurons.coords.length,
        6,
        '2 neurons * 3 components, served from the shared arenas exactly as in single-threaded mode',
      );
      ...
      assert.ok(
        tick.tick >= 0,
        'the automatic tick loop runs against a partitioned simulation too',
      );
      ...
      await client.waitFor((m) => m.type === 'tick');
      await client.waitFor((m) => m.type === 'tick');
      const probeData = (await client.waitFor(
        (m) => m.type === 'probeData',
        3000,
      )) as Extract<ServerMessage, { type: 'probeData' }>;
      assert.equal(
        probeData.neuron,
        a,
        'a probe attached through the server works against a partitioned simulation',
      );
      ...
      assert.equal(
        magic,
        'RASTER',
        'raster export works against a partitioned simulation too',
      );
    } finally {
      client.close();
    }
...
test('the server binds loopback-only (127.0.0.1) by default, never a public address (Requirement 7.6)', async () => {
  ...
  try {
    assert.equal(
      server.host,
      '127.0.0.1',
      'a caller who never names a host must get the loopback-only default, not an accidental public bind',
    );
  } finally {
    await server.close();
  }
