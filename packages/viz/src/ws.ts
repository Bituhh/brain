 ...
 */
export function encodeFrame(opcode: number, payload: Uint8Array, masked = false): Uint8Array {
 ...
 */
export function attachWebSocketServer(httpServer: HttpServer, handlers: WsServerHandlers): void {
  httpServer.on("upgrade", (req: IncomingMessage, socket: Socket, head: Buffer) => {
    const key = req.headers["sec-websocket-key"];
    const upgradeHeader = req.headers["upgrade"];
    if (typeof key !== "string" || typeof upgradeHeader !== "string" || upgradeHeader.toLowerCase() !== "websocket") {
      socket.destroy();
      return;
    }
    ...
    const conn: WsConnection = {
      url: req.url,
      send(bytes) {
        if (!socket.destroyed) socket.write(encodeFrame(OPCODE.binary, bytes));
      },
      close() {
        socket.end();
      },
    ...
    let buffered: Uint8Array = head.length > 0 ? new Uint8Array(head) : new Uint8Array(0);
    let closed = false;
    ...
    const drain = (): void => {
      for (;;) {
        ...
        if (parsed.opcode === OPCODE.close) {
          ...
          if (!socket.destroyed) socket.write(encodeFrame(OPCODE.pong, parsed.payload));
