...
function byData<T extends Element>(
  root: ParentNode,
  attr: string,
  value: string,
): T | null {
...
export function wireControls(
  ...
  byData<HTMLButtonElement>(root, 'data-action', 'pause')?.addEventListener(
    'click',
    () => callbacks.onPause(),
  );
  byData<HTMLButtonElement>(root, 'data-action', 'resume')?.addEventListener(
    'click',
    () => callbacks.onResume(),
  );
  byData<HTMLButtonElement>(root, 'data-action', 'step')?.addEventListener(
    'click',
    () => callbacks.onStepOnce(),
  );
  byData<HTMLButtonElement>(
    root,
    'data-action',
    'request-raster',
  )?.addEventListener('click', () => callbacks.onRequestRaster());
  byData<HTMLButtonElement>(
    root,
    'data-action',
    'request-metrics',
  )?.addEventListener('click', () => callbacks.onRequestMetricsSnapshot());
  ...
  stimulateForm?.addEventListener('submit', (event) => {
    ...
    if (Number.isFinite(index) && Number.isFinite(current))
      callbacks.onStimulate(index, current);
  });
  ...
  strideInput?.addEventListener('change', () => {
    ...
    if (Number.isFinite(value) && value >= 1)
      callbacks.onSetStateStride(Math.floor(value));
  });

  const potentialToggle = byData<HTMLInputElement>(
    root,
    'data-input',
    'show-potential',
  );
  potentialToggle?.addEventListener('change', () =>
    callbacks.onToggleShowPotentialSynapses(potentialToggle.checked),
  );
}
