...
VAL-4 (the acceptance milestone, `docs/findings.md` finding 1) is honestly unmet. Per-phase
status is in [`docs/history.md`](docs/history.md); found-in-code defects are
...
This README is the concise specification for the project: the vision (§1), the numbered
requirements that follow from the evidence base (§3–§9), the invariants that must not be
violated (§10), and the build order (§11). The neuroscience evidence base, the literature
survey, decisions and their rationale, open questions, findings from our own measurements, and
the full build history each have their own file under `docs/`, linked throughout this document
and indexed below. See `CLAUDE.md` for the rule on what goes where.
...
shared vocabulary between this document, the specs, `docs/`, and the code.
...
vision, touch, or language (docs/prior-art.md §2.6). The decisive evidence is the rewiring experiments: route
...
**Amended 2026-09-10 (docs/decisions.md decision 23): stage 1 is no longer purely passive.** Ordering by encoding
...
1. **Locality.** A plasticity rule receives only a local context. It cannot reach the graph.
   **Status (PLAN.md C8, 2026-09-22): unchanged, deliberately, and here is exactly what the
   interface exposes.** A rule is handed `LocalContext` (`pre`, `post`: two `NeuronLocal` copies of
   `last_spike`/`trace`/`rate_estimate`; `modulators`; `tick`) and `SynapseMut` (borrowed
   `permanence`, `weight`, `eligibility`, `last_active`, `eligibility_updated_at`) — no synapse id,
   no arena, no index, and **no compartment or segment information**. C9 needs acetylcholine to
   treat feedforward and recurrent synapses differently; rather than widen this list, the
   *scheduler* resolves a synapse's `segment::SegmentRole` and selects **which configured
   `RuleChain` runs** (`Scheduler::with_plasticity_for_role`). That is still locality because no
   rule gains any input: the anatomy is known only to the component that already holds the whole
   arena and already branched on it, and role-dependent behaviour is expressed as two configured
   rule instances rather than a rule that inspects where it sits.
   `crates/brain-core/tests/plasticity_locality.rs` pins the field list by exhaustive
   destructuring, so adding a field fails to compile there. docs/decisions.md decision 24.
2. **No global gradient.** The only global signals are scalar neuromodulator fields.
   **Status (PLAN.md C9, 2026-09-24): a neuromodulator level now reaches *transmission*, and it is still a
   broadcast scalar.** `transmission.rs`'s gate scales what a delivery carries, selected by the synapse's
   `segment::SegmentRole` — local anatomy, resolved by the scheduler — and multiplied by one level read from the
   shared field. No per-synapse or per-neuron term enters at either end, which is the same standard C2's producer
   was held to (aggregate to a scalar *before* anything reaches the field). docs/decisions.md decision 25.
3. **Sign lives on the neuron.** (Dale's principle.)
   **Status (PLAN.md C9, 2026-09-24): this invariant is now enforced at a second place, by construction.**
   A transmission gate's scale multiplies a delivery's signed current, so a negative scale would turn an
   excitatory contact inhibitory. `TransmissionModulation::with_role` refuses `min < 0`
   (`TransmissionModulationError::NegativeScale`) — deliberately *unlike* C5's amplitude maps, where crossing zero
   is a documented STDP result and is the caller's call (C7 took it). Zero itself is allowed: complete presynaptic
   silencing is the strongest effect the cholinergic preparation reports, and it changes no sign.
...
Language is noted per phase: **[R]** Rust core, **[T]** TypeScript shell. Full "what actually
shipped" status per phase — measured numbers, what's deferred, what's honestly unmet — is in
[`docs/history.md`](docs/history.md), linked from each phase below.
...
- **Phase 0 — skeleton.** Cargo + npm workspace, `brain-core` crate, `brain-napi` bindings,
  ...
  **Resolved:** how randomness is indexed (docs/decisions.md decision 7) — `derive_stream`, a stateless
...
- **Phase 4 — columns and scale.** **[R]** Column primitive, lateral voting, thread
  ...
  Full status: [`docs/history.md`](docs/history.md#phase-4).

- **(No longer a gating phase.) docs/decisions.md decision 22's phase-preservation check — resolved 2026-09-10,
  ...
  note is now in `partition.rs`'s own module docs (where an optimiser chasing docs/open-questions.md item 1's
  ...
  existing per-tick spike-set equality to imply it. Both pass. docs/decisions.md decision 22 is fully resolved as a
...
- **Phase 5 — I/O, grounding and consolidation.** **[T]** `packages/io` (encoders, decoders, the
  ...
  docs/decisions.md decision 21), IO-5's sensorimotor loop, and LRN-10 consolidation over an *abstracted* replay
  source. LRN-12's fast binding is **decided and designed in this phase, not built** — docs/open-questions.md item 2
  ...
  docs/decisions.md decision 23, because what "beats trigram" is evidence *of* depends on whether anything is
  grounding the symbols.
  Full status: [`docs/history.md`](docs/history.md#phase-5).

- **Phase 5.5 — working memory, action selection and reference frames.** **[R]** NET-12 (docs/decisions.md decision 20, moved here from the abandoned Phase 4.5 plan — see above), built first since NET-13 needs its
  multi-tick hold; then NET-13 (docs/decisions.md decision 21), which also needs Phase 5's LRN-11 for the reward
  ...
  first attempt.
  Full status: [`docs/history.md`](docs/history.md#phase-5-5).
...
- **Phase 6 — visualisation.** **[T]** A browser visualiser driven by the native (`napi-rs`)
  ...
  Full status: [`docs/history.md`](docs/history.md#phase-6).
...
- **Phase 7 — scale validation, drift and visual inspection.** **[R]** Closes the empirical gaps
  ...
  - NET-12/13 at a larger, locality-realistic scale — beyond Phase 5.5's 5-neuron clique and
    2-column race. This doubles as the throughput benchmark Phase 4's docs/open-questions.md item 3 deferred:
  ...
  - VAL-3 / semantic drift: today's soak test (`homeostasis.rs`, Requirement 9.3) checks that
    weights stay bounded over a long run, not that predictions stay accurate — docs/findings.md finding 5 flags
  ...
  - VAL-4 resurfaced: either a further tuning pass informed by whatever NET-12/13-at-scale finds
    about the shared predictive substrate, or the demotion decision floated at docs/findings.md finding 1
...
Design questions this project has settled, and why: [`docs/decisions.md`](docs/decisions.md) —
numbered decisions, permanent IDs, never renumbered.

## 12a. Open questions

What is still genuinely undecided: [`docs/open-questions.md`](docs/open-questions.md).

## 13. Prior art — what has already been tried, and what came of it

The literature survey: [`docs/prior-art.md`](docs/prior-art.md) §13.1–13.13. This project's own
findings against that prior art are a separate file, [`docs/findings.md`](docs/findings.md) —
they are measurements, not literature.
