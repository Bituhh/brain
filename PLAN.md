...
items and three copies of one change. **It also carried the staircase check** (added 2026-09-21,
task step 5): C6 and C7 are searches over the knob C5 builds, and if a modulator gain turned out to
be a step function rather than a continuous one — which C3's own results hinted at — then those
searches would report noise as structure. **Settled by C5 (2026-09-21): not a staircase.** On the
...
§12 decision 16 and §13.12 item 18.
...
- USE THE HOOK'S JOINT TIME SCALE (`StdpModulation::joint_time_scale`, FFI `stdpModulation.tauPlus`
  ...
  `clamp(1 + gain x (level - reference), min, max)`, not `constant x level` -- noradrenaline is
  exactly 0 for ~90% of a VAL-4 run (C2, HANDOFF fact 12), and a bare multiplier would make that
  resting state a zero-width window. Choose `reference` deliberately: it is the level at which the
  configured STDP curve is the curve.
...
- THE KNOB IS CONTINUOUS, SO A SEARCH OVER IT IS LEGITIMATE. C7 acts on STDP, which writes WEIGHT,
  ...
  on the PERMANENCE path, which this item does not use.
...
| C7 | not started | | | New 2026-09-20. Cellular and direct — Seol et al. (2007): M1 muscarinic **promotes t-LTD and suppresses t-LTP**; Brzosko (2019) reports mAChR converting pre-before-post LTP into LTD at SC–CA1. ACh can *invert* plasticity's sign, not just scale it. Delicate because the shipped VAL-4 config already holds this channel at a constant 1.0, so B5's pinned figure may need re-reasoning. **Prompt corrected 2026-09-21 by C5:** the knob is continuous (searchable), but **B5's ratio is already the best of four values measured at 15,000 characters** (20.48% against 16.12% / 11.28% / 17.18%), so the comparison against the tuned constant is the real bar; measure at 15,000 characters -- a 6,000-character lead reversed. |
