...
The "no further gain after" column is *not* a plateau in the ordinary sense —
in every case the run subsequently **declines**, and for A and B the decline
takes it far below where it started improving. Read the trajectory below, not
this column alone.
...
The trigram baseline *improves* with exposure (28.40% → 29.20%) while every
...
the sprout/prune churn, not to corpus length — and the occupied count *falls*
...
anticipate a collapse: once accuracy reaches 0 the harness injects
`reward(0.0)` every character, the channel decays to nothing, and the median
measures the dead tail. Recorded rather than replaced. The corrected reading is
**post hoc** and restricted to the early third, while reward was still earned:
