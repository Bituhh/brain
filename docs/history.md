...
The full account of what each phase actually shipped, measured, and deferred,
in build order. `README.md` §11 carries the short, forward-looking plan for
each phase; the detailed status lives here.

**Starts at Phase 4, not Phase 0.** Phases 0–3 never got a separate dated
status write-up — each is just the one- or two-line plan in `README.md` §11,
since this project only started writing a per-phase status once it reached
Phase 4. There is nothing missing here; Phases 0–3's content is complete as
it stands in `README.md` §11.
...
in docs/open-questions.md). Deferred out of this phase: per-column FFI accessors (no column-building FFI exists
in `NativeSimulation` yet) and a full throughput benchmark at the real 100k-neuron/50M-synapse
scale (docs/open-questions.md).
...
`PartitionRuntime::inject_modulator`, which previously reached one partition only (docs/decisions.md decision 21)
— **amended 2026-09-20 (PLAN.md C3), not deleted: that closed LRN-11's *plumbing*, and the signal
travelling through it was the wrong one.** `reward()` injected a raw reward where docs/prior-art.md §2.5 defines
...
the core gained a baseline. Both closed by C3; docs/findings.md finding 16 has the account and the VAL-4
measurement;
...
design decision is recorded as docs/decisions.md decision 8, with no fast-store code written, per Requirement
...
- **VAL-4: not met — and the original 0.67% figure above was itself measured on a network
  ...
  **Further correction, same day: 13.22% was itself measured on a network with a second
  predictive-learning gap, now also fixed — see docs/findings.md finding 6.** `GraphBuilder::connect` (the
  ...
  the topology-level unit tests added alongside it, docs/findings.md finding 6), and the mechanism it restores
  (independent per-segment coincidence detection, docs/prior-art.md §2.3/NEU-5) is now genuinely running end-to-end
  for the first time in this milestone's history — it simply does not help *this* network's
  accuracy, and by docs/findings.md finding 7's density-artefact evidence, appears to make the network's tick-2
  ...
  **Superseded by the tuning passes that followed, 2026-09-15/16 — see §12 decisions 12 and 13.**
  ...
  character" baseline docs/findings.md finding 7 measured, which every earlier figure in this document failed
  to clear.

  **Consolidation now runs inside a real experiment, and it does not help — 2026-09-19, PLAN.md
  C1.** This phase shipped LRN-10 with no streaming caller anywhere: docs/prior-art.md §2.9 calls an offline phase
  ...
  four mechanism findings behind the number, and what stays open: docs/findings.md finding 13 and docs/open-questions.md item 3.
...
- **NET-13: met.** `crates/brain-core/tests/action_selection.rs` and
  ...
  docs/decisions.md decision 21 broadcast bug. The FFI surface (`GatingGroupConfig`, `buildColumns`'s new
...
- **LRN-12: not built.** docs/decisions.md decision 9 records why: none of the above needed
...
NET-10 growth-regression investigation (docs/findings.md finding 10, Phase A) diagnosed growth as inert (a
bootstrapping deadlock, later found to be blocked at its root by item 12) rather than harmful, and
the broader `segmentsPerNeuron` x `targetRate` retuning search (docs/findings.md finding 10, Phase B) found
...
- **Throughput benchmark: run, target still not met, but the shape of the result changed.** See
  docs/open-questions.md item 1's own new entry for the full numbers — `bench_locality_realistic_synaptic_events_per_second`
...
- **NET-10 wired live: met, invariant 10 is now actually true for neuron count.**
  ...
  - **NET-10 functional capacity (PLAN.md item B2, 2026-09-14): re-measured, still not met.** The
    ...
    capacity: re-running docs/findings.md finding 10's own six-condition script after B1 (the weight/permanence
    ...
    entire 15,000-character run, in every condition tried. See docs/findings.md finding 10's own 2026-09-14
  ...
  - **NET-10 functional capacity (PLAN.md item B3, 2026-09-14): the eligibility and
    ...
    docs/findings.md finding 10's 2026-09-14 update for the full account. On the real network, grown neurons
    ...
    grown neurons. **VAL-4 result (5-seed × 6-condition battery, docs/findings.md finding 10's own protocol,
    ...
    fix. See docs/findings.md finding 10's 2026-09-14 update for the full table, per-window instrumentation, and
  ...
  - **NET-10 reachability (PLAN.md item C4, 2026-09-21): the last structural gap is closed —
    ...
    with" answered by index and growth appends past every original's block. docs/decisions.md decision 15
    ...
    monotonically worse as the radius widens. docs/findings.md finding 17 has the tables, the no-growth control
...
- **Canonical "everything on" brain constructor (PLAN.md item A1): built.**
  ...
  dead neuromodulator channels (docs/findings.md findings 11/13) sit unnoticed in a tree with a strong test
  ...
  **`weight` split from `permanence` — closed 2026-09-14, PLAN.md item B1, the largest change in
  the plan.** docs/findings.md finding 12 found that `SynapseArena` aliased SYN-3's structural gate and docs/prior-art.md §2.5's
  ...
  `StructuralPlasticity`. See docs/decisions.md decision 11 for the full design, and its own closing paragraph
  ...
  that gate directly; see docs/findings.md finding 10's 2026-09-14 update for the full finding. Format-version-9
  snapshots
  ...
  not a regression. See docs/findings.md finding 12 for the finding's own closing outcome note.
